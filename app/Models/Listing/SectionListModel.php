    ...
    public function insertDraftList($list_dt, $board_type, $mainhead, $ucode)
    {
       $sql = "INSERT INTO draft_list (diary_no, next_dt_old, conn_key, list_type, board_type, usercode, ent_time)
        ...
        WHERE
            ...
            AND (
                TRIM(LEADING '0' FROM SPLIT_PART(m.fil_no, '-', 1))::INTEGER IN 
            ...
            AND m.conn_key ~ '^[0-9]+$' -- ensure conn_key is numeric
            AND (
            ...
            AND NOT EXISTS (
                ...
                WHERE d.diary_no = m.diary_no
                  AND d.next_dt_old = h.next_dt
                  AND d.conn_key = m.conn_key::INT
                  AND d.list_type = 1
                  AND d.board_type = '$board_type'
            )";
