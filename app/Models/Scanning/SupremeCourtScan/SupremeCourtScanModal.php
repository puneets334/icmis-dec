...
class SupremeCourtScanModal extends Model
{
    ...
    public function getIndexingReport($from_date, $to_date, $status)
    {
        ...
        $builder->select('a.diary_no, lct_dec_dt, l_dist, ct_code, l_state, b.name AS state_name, 
                        a.lct_casetype, a.lct_caseno, a.lct_caseyear, a.lower_court_id, a.is_order_challenged, 
                        a.full_interim_flag, a.judgement_covered_in, e.casetype_id, ihd.conformation,');
        ...
        if ($status == '1') {
            $builder->where('ihd.conformation', 1);
        } elseif ($status == '2') {
        ...
        $query = $builder->get();
        // echo $builder->getCompliedSelect();
