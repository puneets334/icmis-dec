...
class EarliercourtModel extends Model
{
    ...
    {
        $builder = $this->db->table("caveat_lowerct$is_archival_table as a");
    ...
    transfer_court');
        ...
        $builder->join("caveat$is_archival_table e", 'e.caveat_no = a.caveat_no');
    ...
    {
        $builder = $this->db->table("caveat_lowerct_judges");
    ...
    {
        $builder = $this->db->table("caveat_lowerct as l");
        $builder->select([
            'id',
            'lower_court_id',
            '(CASE 
        ...
        $builder->join('caveat_lowerct_judges as lj', 'l.lower_court_id = lj.lowerct_id', 'RIGHT');
...
// Subquery to get caveat_no values from caveat_diary_matching
        /* $subquery = $this->db->table('caveat_diary_matching')
             ->select('caveat_no')
             ->where('diary_no', $caveat_no)
             ->where('display', 'Y');*/

        // Build the main query
        $query = $builder->select('*')
            //->whereIn('caveat_no', $subquery)
            ->whereIn('caveat_no', $caveat_no)
    ...
    {
        $builder = $this->db->table("caveat_lowerct as a");
        $builder->select([
            'lower_court_id',
            'transfer_court',
            'transfer_case_type',
    ...
    {
        $builder = $this->db->table("caveat$is_archival_table as c");
        $builder->select("c.*,b.name,b.aor_code");
        $builder->join('master.bar b',  'c.pet_adv_id=b.bar_id', 'left');
        $builder->where('c.caveat_no', $caveat_no);
        $query = $builder->get();
