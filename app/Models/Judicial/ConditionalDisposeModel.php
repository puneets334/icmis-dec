...
class ConditionalDisposeModel extends Model{
    ...
    ,$case_type_disp=0,$case_number_disp=0,$case_year_disp=0,$diary_number_disp=0,$diary_year_disp=0){
        ...
        if(($case_type_list!=0 && $case_type_list!='' ) && ($case_number_list!=0 && $case_number_list!='') && ($case_year_list!==0 && $case_year_list!='')){
            // $condition=" cast(substring(active_fil_no,1,2) as unsigned)=$case_type_list and 
            // active_reg_year=$case_year_list and (cast(substring(active_fil_no,4,6)
            // as unsigned)=$case_number_list or $case_number_list between 
            ...
            // $condition=" cast(substring(active_fil_no,1,2) as unsigned)=$case_type_disp and 
            // active_reg_year=$case_year_disp and (cast(substring(active_fil_no,4,6)
            // as unsigned)=$case_number_disp or $case_number_disp between 
            // cast(substring(active_fil_no,4,6) as unsigned) and 
            // cast(substring(active_fil_no,11,6) as unsigned))";
        ...
        // $sql="select m.diary_no as case_diary,m.c_status,concat(m.pet_name,' Vs ',m.res_name) as case_title,dacode from main m where " ;
    ...
    function get_conn_details($diarynumber)
    {
        ...
        // $sql="select count(diary_no) as conn from main where conn_key=$diarynumber and diary_no!=conn_key";
        // $query = $this->db->query($sql);
    ...
    function update_case_HighCourt($list_diary, $dispose_hCourt,$court_type,$connected,$usercode)
    {
        ...
        if($this->check_case($list_diary)==true) 
        {
            ...
            $subquery->select("fil_no,conn_key,reason,fil_no2,remove_def,remove_def_dt,ent_dt,rgo_updated_by,now(),hcourt_no,court_type");

            $subquery->where("fil_no", $list_diary);
            $query = $subquery->get();
            if($query->getNumRows() >= 1) 
            {
                $insertData = $query->getResultArray(); //pr($insertData);

                foreach ($insertData as &$row) {
                    $row['removed_by'] = $usercode;  // Add usercode
                    $row['removed_on'] = $row['now']; // Add current timestamp
                    unset($row['now']); // Remove 'now' field to match table schema
                }
                
                $this->db->table('rgo_default_history')->insertBatch($insertData);
            }
        ...
        if ($connected == '0'){
            ...
            $this->db->table('rgo_default')->insert($data); 
        }
        else 
        {
            ...
            $sql = "INSERT INTO rgo_default (fil_no, conn_key, hCourt_no, ent_dt, rgo_updated_by, court_type, reason)
                    SELECT diary_no, ?, ?, now(), ?, ?, ''
                    FROM main
                    WHERE conn_key = ?";
            
            $this->db->query($sql, [$list_diary, $dispose_hCourt, $usercode, $court_type, $list_diary]);
        }
