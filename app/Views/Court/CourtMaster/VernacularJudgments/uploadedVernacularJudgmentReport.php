                        ...
                        <div class="col-md-12">
                            <div class="row">
                                <div class="col-md-12 mt-3">
                                    ...
                                    echo form_open(base_url('Court/CourtMasterController/vernacularJudgmentReport'), $attribute);
                                    ?>
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <label for="from_date" >From Date:</label>
                                            <input type="date" id="fromDate" name="fromDate" class="form-control datepick"  placeholder="From Date" >
                                        </div>
                                        <div class="col-sm-3">
                                    ...
                                    <div class="row mt-3">
                                        <div class="text-danger text-bold center-block"><?=$msg?></div>
                                    </div>
                                    <?php
                                    if (isset($uploadedVernacularJudgmentsReport) && is_array($uploadedVernacularJudgmentsReport) && sizeof($uploadedVernacularJudgmentsReport) > 0)
                                    {
                                        ?>
                                        <div id="printable" class="box box-danger mt-3">
                                            <caption><h4 style="text-align: center;">  List of Uploaded Vernacular Judgment between &nbsp;<strong><?=date('d-m-Y',strtotime($param[0]));?></strong>  to  <strong><?=date('d-m-Y',strtotime($param[1]));?></strong> </h4></caption>
                                            <?php
                                            if($app_name=='vernacularJudgmentReport')
                                            {
                                                ?>
                                                <table id="reportTable1" class="table table-striped table-hover">
                                                    ...
                                                    <tr>
                                                        <th width="4%">#</th>
                                                        <th width="8%">High Court</th>
                                                        <th width="8%">State</th>
                                                        <th width="8%">Language</th>
                                                        <th width="15%">Case Details</th>
                                                        <th width="8%">Judgment Date</th>
                                                        <th width="8%">File</th>
                                                        <th width="8%">Uploaded By</th>
                                                        <th width="8%">Uploaded On</th>
                                                    </tr>
                                                    </thead>
                                                    <tbody>
                                                    <?php
                                                    $s_no=1;
                                                    foreach ($uploadedVernacularJudgmentsReport as $case)
                                                    {
                                                        ?>
                                                        <tr>
                                                            <td><?=$s_no;?></td>
                                                            <td><?= !empty($case['highCourt']) ? $case['highCourt'] : '' ?></td>
                                                            <td><?= !empty($case['stateName']) ? $case['stateName'] : '' ?></td>
                                                            <td><?= !empty($case['language']) ? $case['language'] : '' ?></td>
                                                            <td>
                                                                <?= !empty($case['caseNo']) ? $case['caseNo'] : '' ?>
                                                                <br>
                                                                <?= !empty($case['causeTitle']) ? $case['causeTitle'] : '' ?>
                                                            </td>
                                                            <td><?= !empty($case['judgmentDate']) ? $case['judgmentDate'] : '' ?></td>

                                                            <td>
                                                                <?php if(isset($case['filePath']))
                                                                {

                                                                    ?>

                                                                <a href="/supreme_court/<?= $case['filePath'] ?>" target="_blank">
                                                            ...
                                                            <td><?= !empty($case['uploadedBy']) ? $case['uploadedBy'] : '' ?></td>
                                                            <td><?= !empty($case['uploaded_on']) ? $case['uploaded_on'] : '' ?></td>
                                                        </tr>
                                                        <?php
                                                        $s_no++;
                                                    }   //for each
...
<script>
    function check()
    {
        ...
        if (fromDate && toDate)
        {
            ...
            var date1 = new Date(fromParts[2], fromParts[1] - 1, fromParts[0]);
            var date2 = new Date(toParts[2], toParts[1] - 1, toParts[0]);
