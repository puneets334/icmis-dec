<?=view('header'); ?>
<style>
    /* input[type="text"], input[type="date"], input[type="email"], input[type="tel"], input[type="number"], input[type="url"], input[type="password"], input[type="search"], select, textarea 
    {
	    border: 1px solid #e1e1e1 !important;
	    width: 100% !important;
	    height: 38px !important;
	    padding: 5px 10px !important;
	    border-radius: 0 !important;
	} */

	.form-control, .btn {
		font-size: 14px !important;
	}
	* {
        box-sizing: border-box;
    }

    .my_B {
    font-size: 16px;
    color: Black;
    font-weight: bold;
   }

   .dl-horizontal dd {
        margin-left: 180px;
        line-height: 0.2em;
    }
    dl {
    margin-top: 0;
    margin-bottom: 20px;
}

.dl-horizontal dt {
        float: left;
        width: 160px;
        overflow: hidden;
        clear: left;
        text-align: right;
        text-overflow: ellipsis;
        white-space: nowrap;
    }
    </style>

<section class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header heading">
                        <div class="row">
                            <div class="col-sm-10">
                                <h3 class="card-title">Mention Memo >> Search for M.M. </h3>
                            </div>
                            <?= view('Filing/filing_filter_buttons'); ?>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="card-body">
 


                                <form id="myform" action="#" enctype="multipart/form-data" method="POST" class="form-inline" >
                                <?php
                                    $attribute = array('class' => 'form-horizontal','name' => 'report', 'id' => 'report', 'autocomplete' => 'off');
                                    echo form_open(base_url('Court/CourtMentionMemoController/mention_memo_report'), $attribute);
                                    ?>
                               <table id="example1" class="table table-bordered table-striped mt-2">
                              <tbody>
                            <tr> 
                                <td>Cause Title</td>
                                <td><?php echo $search_details['pet_name'];
                                  if($search_details['pno']==2) echo " <span style='color:#72bcd4'> AND ANR</span>"." ";
                                   else if($search_details['pno']>2) echo " <span style='color:#72bcd4'> AND ORS</span>"." "; ?>
                                  <b>VS</b>
                                  <?php echo ' '.$search_details['res_name'];
                                    if($search_details['rno']==2) echo " <span style='color:#72bcd4'> AND ANR</span>";
                                 else if($search_details['rno']>2) echo " <span style='color:#72bcd4'> AND ORS</span>";?></td>
                            </tr>
                         <tr>
                            <td>Diary No.</td><td><?php echo $search_details['diary_number'] ?>/<?php echo $search_details['diary_year'] ?>,<b> Nature: </b><?php echo $search_details['short_description'] ?></td>       
                         </tr>
                        <tr>
                            <td>Receive Date</td><td><?php if($search_details['diary_no_rec_date']!='') echo date('d-m-Y h:i:s A',strtotime($search_details['diary_no_rec_date']));?></td>
                        </tr>
                        <tr>
                            <td>Case No.</td><td><?php echo $search_details['short_description'].substr($search_details['active_fil_no'],3).'/'.$search_details['active_reg_year'];?></td>
                        </tr>
                     <tr>
                        <td>Registration Date</td>
                        <td>
                            <?php 
                                $active_fil_dt = $search_details['active_fil_dt'] ?? null;
                                $active_date = "N/A";
                                if ($active_fil_dt && $active_fil_dt !== '') {
                                    $active_fil_dt_check = strtotime($active_fil_dt);
                                    if ($active_fil_dt_check !== false) {
                                        $active_date = date('d-m-Y h:i:s A', $active_fil_dt_check);
                                    }
                                }
                                echo $active_date;
                                ?>
                        </td>
                    </tr>
                    
                     <tr><td>Status</td><td><?php if($search_details['c_status']=='P') echo "Pending"; else if($search_details['c_status']=='D') echo "Disposed"; ?>
                    </td>
                    </tr>
                        <tr><td>Pet. Advocate(s)</td><td><?php echo $padvname; ?></td></tr>
                        <tr><td>Resp. Advocate(s)</td><td><?php echo $radvname; ?></td></tr>
                        <tr>
                            <td>State and Bench</td>
                            <td>
                                <?php 
                                echo isset($search_details_data['agency_state']) ? $search_details_data['agency_state'] : '';
                                //if($search_details['from_court']!=3) echo ' = '.$search_details_data['agency_name']; 
                                if ($search_details['from_court'] != 3 && isset($search_details_data['agency_name'])) {
                                    echo ' = ' . $search_details_data['agency_name'];
                                }
                                ?>
                            </td>
                                
                        </tr>
                    <tr>
                        <td>Category</td>
                        <td><?php echo $catName; ?>
                        <?php echo $newCatName;?>
                        </td>
                    </tr>
                    <tr>
                        <td>Listing Date</td>
                        <td>
                            <?php
                            if(isset($search_details['next_dt']) && ($search_details['next_dt']>=date('Y-m-d'))){
                                if($search_details_data['main_supp_flag'] ==1 || $search_details_data['main_supp_flag'] ==2){
                                    if($search_details_data['roster_id']>0 && $search_details_data['clno']>0){
                                        echo "Listed On ".date('d-m-Y',strtotime($search_details['next_dt'])).' Before ';
                                        if($search_details_data['board_type']=='J')
                                            echo "Hon' Coram";
                                        else if($search_details_data['board_type']=='C')
                                            echo "Chamber";
                                        else if($search_details_data['board_type']=='R')
                                            echo "Registrar";
        
                                        echo " ".$search_details_data['jname'];
                                    }
                                }
                                else {
                                    echo "Not Listed";
                                }
                            }
                            ?>
                        </td>
                    </tr>
                 <tr><td>Caveator Adv(s)</td><td style="color: #4f8ad2"><?php if (!empty($results)) {
                            foreach ($results as $row) {
                                echo $row['name'] . '<br>';
                            }
                        } else {
                            echo "Not Found";
                        } ?>
                 </td></tr>


                 <tr><td>Proof of Service</td>
                 <td><?php echo $proof_of_service; ?></td></tr>
                  <tr><td>Pending IA(s)</td><td><?php if (!empty($pending_Ai)):  ?>
                            <table>
                                <?php foreach ($pending_Ai as $row): ?>
                                    <tr>
                                        <td><?= $row['docnum'] . '/' . $row['docyear']; ?></td>
                                        <td>
                                            <?php if ($row['doccode1'] == 19): ?>
                                                <?= $row['other1']; ?>
                                            <?php else: ?>
                                                <?= $row['docdesc']; ?>
                                            <?php endif; ?>
                                            Filed on <?= date('d-m-Y', strtotime($row['ent_dt'])); ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </table>
                        <?php else: ?>
                            <p>N.A.</p>
                        <?php endif; ?>
                </td></tr>
                        <tr>
                        <td></td>
                        <td>
                        <?= $f_get_category_judge; ?>
                        <?= $f_get_ndept_judge; ?>
                        <?= $f_get_ntl_judge; ?>
                <?php if (!empty($not_before)): ?>
                        <div align="center" style="border: 1px solid black;">
                            <table class="table_tr_th_w_clr c_vertical_align" cellspacing="5" cellpadding="8">
                                <tr>
                                    <th colspan="5"><span id="edit_lb"></span>Already Entries of List before and not before and coram</th>
                                </tr>
                                <tr>
                                    <th>Sr.</th>
                                    <th>Before/Not before</th>
                                    <th>Hon. Judge</th>
                                    <th>Reason</th>
                                    <th>Entry Date</th>
                                </tr>
                                <?php $s = 1; ?>
                                <?php foreach ($not_before as $row): ?>
                                    <tr>
                                        <td><?= $s++; ?></td>
                                        <td><?= $row['notbef'] ?? '';?></td>
                                        <td><?= $row['jname'] ?? ''; ?></td>
                                        <td><?= $row['res_add'] ?? ''; ?></td>
                                        <td><?=  !empty($row['ent_dt']) ?  date('Y-m-d',strtotime($row['ent_dt'])) : ''; ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </table>
                        </div>
                        <br>
                    <?php else: ?>
                        <div style="text-align:center;padding:10px;color:red;">
                            <span id="edit_lb"></span>LIST BEFORE/NOT BEFORE/CORAM NOT FOUND
                        </div>
                    <?php endif; ?>
                    </td></tr>
                    <tr>
                       <td>Tagged Matters	
                       </td>
                       <td class="text-danger">CONNECTED MATTERS NOT FOUND

                        </td>
                    </tr>

                    </tbody>

                  </table>
 

                         <?php form_close(); ?>

                      </form>
                      <hr>
                      <button type="button" class="btn btn-primary" id="submit" onclick="getDetails()" style="margin: 0 auto;display: block;">View Similarities</button>
                      <hr>
                        <div id="div_result"></div>
                                        </div>
                                    </div>
                                </div>
                               
              </div>
           </div>
        </div>

 </div>
</section>

<script>

function getDetails()
{
    var diary_number = '<?php echo $search_details['diary_number'] ?>';
    var diary_year = '<?php echo $search_details['diary_year']?>';
    if (diary_number.length == 0) {
        alert("Please enter Diary number");
        $("#diary_number").focus();
        validationError = false;
        return false;
    }else if (diary_year.length == 0) {
        alert("Please select Diary year");
        $("#diary_year").focus();
        validationError = false;
        return false;
    }
    var CSRF_TOKEN = 'CSRF_TOKEN';
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    $.ajax({
        url: base_url+'/Filing/Similarity/viewSimilarity',
        cache: false,
        async: true,
        data: {CSRF_TOKEN: CSRF_TOKEN_VALUE,diary_number: diary_number,diary_year:diary_year},
        beforeSend: function () {
            //$('#div_result').html('<table widht="100%" align="center"><tr><td><img src="../images/load.gif"/></td></tr></table>');
            $('#div_result').html('<div style="margin:0 auto;margin-top:20px;width:15%"><img src="' + base_url + '/images/load.gif"/></div>');
        },
        type: 'POST',
        success: function(data, status) {
            updateCSRFTokenSync();
            $('#div_result').html(data);
        },
        error: function(xhr) {
            updateCSRFTokenSync();
            //alert("Error: " + xhr.status + " " + xhr.statusText);
        }

    });
}

getDetails();

    $(document).ready(function() {
                    $(function () {
                        $('.datepick').datepicker({
                            format: 'dd-mm-yyyy',
                            autoclose:true
                        });
                    });
                    $('#example1,#example2,#reportTable3,#reportTable4').DataTable( {
                        dom: 'Bfrtip',
                        "scrollX": true,
                        "bProcessing"   :   true,
                        "ordering": false,
                        buttons: [
                            'excelHtml5',
                            {
                                extend: 'pdfHtml5',
                                pageSize: 'A3',
                                customize: function ( doc ) {
                                    doc.content.splice( 0, 0, {
                                        margin: [ 0, 0, 0, 5 ],
                                        alignment: 'center',
                                        image: 'data:image/png;base64,'
                                    });
                                    doc.watermark = {text: 'SUPREME COURT OF INDIA', color: 'blue', opacity: 0.05}
                                }
                            }

                        ],
                        lengthMenu: [
                            [ 10, 25, 50, -1 ],
                            [ '10 rows', '25 rows', '50 rows', 'Show all' ]
                        ],

                        "footerCallback": function ( row, data, start, end, display ) {
                            var api = this.api(), data;

                            // Remove the formatting to get integer data for summation
                            var intVal = function ( i ) {
                                return typeof i === 'string' ?
                                    i.replace(/[\$,]/g, '')*1 :
                                    typeof i === 'number' ?
                                        i : 0;
                            };
                            // Total over all pages
                            total = api
                                .column( 4 )
                                .data()
                                .reduce( function (a, b) {
                                    return intVal(a) + intVal(b);
                                }, 0 );

                            // Total over this page
                            pageTotal = api
                                .column( 4, { page: 'current'} )
                                .data()
                                .reduce( function (a, b) {
                                    return intVal(a) + intVal(b);
                                }, 0 );
                            // Update footer
                            $( api.column( 4 ).footer() ).html(pageTotal +' ('+ total +' Total)');
                        }

                    } );
                } );

                $(".alert").delay(4000).slideUp(200, function() {
                    $(this).alert('close');
                });
                $('a.delete').on('click', function() {
                    var choice = confirm('Do you really want to delete this record?');
                    if(choice === true) {
                        return true;
                    }
                    return false;
                });
            </script>