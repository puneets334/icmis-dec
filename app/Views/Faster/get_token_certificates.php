<?php
$certificate_result = file_get_contents("http://".get_client_ip().":8100/api/v1/certificates_list");
//$certificate_result = file_get_contents("http://10.40.186.15:8100/api/v1/certificates_list");
$json = json_decode($certificate_result, true);
if(count($json)>0){
    ?>

    <div class="container text-left">
    <form class="form-inline" id="token_certificate_form" name="token_certificate_form" enctype="multipart/form-data">
        ...
        <input type="hidden" name="url_pdf_embed_path" id="url_pdf_embed_path" value="<?=urlencode($file_path.$file_name)?>">
            ...
            <button type="button" id="sign_now" name="btn_sign2" class="form-control btn btn-default btn_token_pdf_sign">Sign Now</button>
