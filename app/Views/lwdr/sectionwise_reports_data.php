...
if (count($case_result) > 0 && is_array($case_result) && !empty($case_result)) {
	...
	if ($param[0] == 0) { 
			$title .= 'List of Pending Matters of Section : '.$case_result[0]['user_section'].' pertaining to Category: ';
			if ($param[6] != 0){
				$title .= $case_result[0]['subject_category'];
			}else{
				$title .= $case_result[0]['sub_name1'];
			}
	} else if ($param[0] == 1 && $param[1] == 'J') {
			$title .= 'List of Pending Matters of Section : ' .$case_result[0]['user_section']." likely to be listed before Hon'ble Court Report";

	} else if ($param[0] == 1 && $param[1] == 'C') { 		 
          $title .= 'List of Pending Matters of Section : '.$case_result[0]['user_section'].' likely to be listed before Chamber Report';    
    
	} else if ($param[0] == 1 && $param[1] == 'R') {} else if ($param[0] == 1 && $param[1] == 'R') {                
           
          $title .= 'List of Pending Matters of Section : '.$case_result[0]['user_section'].' likely to be listed before Registrar Court Report:';		   
 
    } else if ($param[0] == 2 && $param[2] == 'F') {
             
		 $title .= 'List of Pending Matters of Section : '.$case_result[0]['user_section'].' likely to be listed in Future Dates';

	} else if ($param[0] == 2 && $param[2] == 'P') { 

         $title .= 'List of Pending Matters of Section : '.$case_result[0]['user_section'].' that were lastly listed';
     
	} else if ($param[0] == 4) {
       $title .=  'Section : '.$case_result[0]['user_section'].' Matters disposed between '.date("d-m-Y", strtotime(strtr($param[3], '/', '-'))).' and '. date("d-m-Y", strtotime(strtr($param[4], '/', '-')));
...
<caption>
            <h2 style="text-align: center;">
                <?php if ($param[0] == 0) { ?>
                    ...
                    <strong><?php if ($param[6] != 0) echo $case_result[0]['subject_category'];
                ...
                ?>
                    <tr>
                        ...
                        <td><?php if ($result['diary_no_rec_date'] == '' || $result['diary_no_rec_date'] == null || $result['diary_no_rec_date'] == "0000-00-00 00:00:00")
                                echo $result['diary_no_rec_date'] = '0000-00-00';
                            else
                                echo date('d-m-Y', strtotime($result['diary_no_rec_date'])); ?>
                        </td>
                        <?php if (isset($_POST['reportType']) && $_POST['reportType'] != 4) { ?>
                            <td><?php if ($result['active_fil_dt'] == '' || $result['active_fil_dt'] == null || $result['active_fil_dt'] == "0000-00-00 00:00:00")
                                    echo $result['active_fil_dt'] = '0000-00-00';
                                else
                                    echo date('d-m-Y', strtotime($result['active_fil_dt'])); ?>
                            </td>
                        <?php } ?>
