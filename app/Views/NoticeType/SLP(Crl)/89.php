...
In the Petition above mentioned Notice(s) was sent to the following person(s) by Speed Post Under postal receipt number(s) at the address(es) given against him (them) through your Post Office on  below mentioned Dates. Tracking report status is not up to date. I am, therefore, to request you to institute necessary enquiry in the matter and in case the said notice(s) has been duly served, duplicate certificate acknowledgement(s) showing the date(s) on which it was served may kindly be sent to this Registry at an early date.
</font>
...
<div style="margin-left: 30px"><?php
  ...
  <?php
 $rs = unserved_parties($dairy_no, 'R');
 
 ?>

<?php

 // echo "total number of rows are :-".mysql_num_rows($rs);
  // $total=mysql_num_rows($rs); 
  
  
  if(!empty($rs))
  {
      echo "<table style='font-size: 13pt;vertical-align: top' border='1' width='100%' >";
   echo " <tr><th>Name and Address of the Person to whom notice was sent </th><th>SPEED POST NO.</th><th>DISPATCH DATE</th>";
   echo " </tr>";
      ...
      foreach ($rs as $row) {
        ...
        echo "</td> ";
        echo "<td> ";
        echo date('dS F, Y', strtotime($row['dispatch_dt']));
        // echo $row[5];
        //  echo $row[5];
        echo "</td> ";
        echo "</tr>";  
