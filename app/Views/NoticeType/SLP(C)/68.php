...
<!--   </p>-->
   ...
   if($row['individual_multiple']==1)
   {
        ...
        $row_check = $builder->select('ind_dep')
            ->where([
                'diary_no' => $dairy_no,
                'pet_res'  => $row['pet_res'],
                'pflag'    => 'P',
        ...
        if ($row_check && $row_check->ind_dep != 'I') {
            $ind_org = 1;
        }
  ...
  $no_of_petitiner='';
   ...
   $doc_names='';
     for ($index22 = 0; $index22 < count($get_application_registration_all); $index22++) {
        ...
        ?>
      WHEREAS the  appeal above-mentioned (copy enclosed) filed in the 
      Registry by <b style="font-size: 13pt" face= "Times New Roman"><?php echo $get_petitioner_advocate; ?></b>, Advocate, on 
      behalf of the Appellant above-named were 
      listed for hearing before this Court on the <b style="font-size: 13pt" face= "Times New Roman"><?php echo $listed_dt ?></b>, when the Court was pleased 
...
<font  style="font-size: 13pt" face= "Times New Roman">
     NOW, THEREFORE, TAKE NOTICE that the above appeal  above-mentioned will be posted for 
     hearing before this Court in due course when you may appear before the Court either in person 
     or through an Advocate-on-record of this Court duly appointed by you and show cause to 
     the Court on the day that may subsequently be specified as to why the leave to appeal may not be granted.  
             </font>
    </p>
     ...
     <p style="text-indent: 40px;padding: 0px 2px 0px 2px;margin: 10px 0px 0px 0px" align="justify">
         <font  style="font-size: 13pt" face= "Times New Roman">
     You may file your affidavit in opposition to the petition  within 30 days from the date of 
     receipt of  notice or not later than 2 weeks before the date appointed for hearing, 
     whichever be earlier, but shall do so only by setting out the grounds in opposition to the 
     questions of law or grounds set out in the Appeal and may produce such pleadings and documents 
     filed before the Tribunal against whose order the appeal is filed.	
             </font>
    </p>
    <?php // } ?>
