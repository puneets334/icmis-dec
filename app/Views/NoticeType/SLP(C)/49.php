...
<u style="font-size: 13pt"  face= "Times New Roman">IN</u>
   ...
   ?>  
      <p style="color: #000000;margin: 0px;padding: 0px 2px 0px 42px;width: 50%;" >
        ...
        <b> <font style="font-size: 13pt;text-transform: uppercase;"  face= "Times New Roman" >
            <?php
              echo $tw_sn_to; ?></b>, 
    </p>
    <?php if($address_m!='') { ?>
...
<!--    <p align="center" style='margin: 0px;padding: 10px 0px 0px 0px;clear: both' >
  ...
  $first_listed_date=date('dS F, Y', strtotime($get_first_listed_date));
   $diary_rec_dt=$new_date = date('dS F, Y', strtotime(get_diary_rec_date($dairy_no)));
        ?>
In supersession of this registry's notice dated _________, TAKE NOTICE that the Special Leave Petition above mentioned will be listed before the Hon'ble Court for hearing on _________ instead of _________ and will be taken up by the Court on that date at 10.30'O Clock in the forenoon or so soon thereafter as may be convenient to the Court.
</font>
     </p>
