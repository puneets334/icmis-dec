...
<div  style="padding-left: 2px;padding-right: 2px;margin-left: 48px" width="100%">
    <div style="width: 48%;border-collapse: collapse;border: 1px solid black;float: left;font-size: 13pt;padding: 5px" border="1" >
        <?php echo get_text_msg();?>
     </div>
    <div style="width: 48%;float: right;font-size: 13pt;text-align: center">
        <b><i><u>Delivery Mode:
                    <?php
 ...
 $lct_caseno=$lower_court[$index1][4];
  $lct_caseyear=$lower_court[$index1][5];
   $get_order_date= get_order_date($dairy_no);
           ?>
        <p align="center" style='margin: 0px;padding: 5px 0px 0px 0px;' >
<u style="font-size: 13pt"  face= "Times New Roman">IN</u>
     ...
     <p align="center" style='margin: 0px;padding: 5px 0px 0px 0px;' >
       ...
       for ($index2 = 0; $index2 < count($ex_lct_caseno); $index2++) {
           if($index2>0){ echo ',';}
       ?>
     <u><b style="font-size: 13pt"  face= "Times New Roman"> <?php echo $ex_skey[$index2] ?> </b>  <b style="font-size: 13pt"  face= "Times New Roman"><?php echo $ex_lct_caseno[$index2]; ?></b> of <b style="font-size: 13pt"  face= "Times New Roman"><?php echo $ex_lct_caseyear[$index2]; ?> </b></u> <?php } ?>
    </p>
       <?php
     ...
     <p align="justify" style='margin: 10px;padding: 2px 0px 0px 0px;font-size: 13pt'>
       <?php
     $lower_court= lower_court_not_challanged_conct($dairy_no);
 ...
 $lct_caseno=$lower_court[$index1][4];
  $lct_caseyear=$lower_court[$index1][5];
   ...
   $get_order_date= get_order_date($dairy_no);
           ...
           ?>
<!--           <div style="font-size: 13pt;margin-bottom: 10px">(From the Judgment and Order dated <b style="font-size: 13pt"  face= "Times New Roman"><?php echo date('dS F, Y', strtotime($get_order_date[0])); ?></b>  in 
       <?php
       $ex_skey=  explode(',',$skey );
       $ex_lct_caseno=explode(',',$lct_caseno );
       $ex_lct_caseyear=explode(',',$lct_caseyear );
        $ex_lct_casetype=explode(',',$lct_casetype );
        $lwct_diary_no=array();
       for ($index2 = 0; $index2 < count($ex_lct_caseno); $index2++) {
           
        $get_diary_case_type= get_diary_case_type($ex_lct_casetype[$index2],$ex_lct_caseno[$index2],$ex_lct_caseyear[$index2]);
         
          if(!in_array($get_diary_case_type, $lwct_diary_no) )
          {
           
           $lower_court= lower_court_conct($get_diary_case_type); 
           for ($index1 = 0; $index1 < count($lower_court); $index1++) {
 $judgement_dt=$new_date = date('dS F, Y', strtotime($lower_court[$index1][0]));
 $agency_name=$lower_court[$index1][2];
  $skey=$lower_court[$index1][3];
  ...
  $lct_caseyear=$lower_court[$index1][5];
   $get_order_date= get_order_date($dairy_no);
       ...
       for ($index2 = 0; $index2 < count($ex_lct_caseno); $index2++) {
           if($index2>0){ echo ',';}
       ?>
       <b style="font-size: 13pt"  face= "Times New Roman"> <?php echo $ex_skey[$index2] ?> </b>  <b style="font-size: 13pt"  face= "Times New Roman"><?php echo $ex_lct_caseno[$index2]; ?></b> of <b style="font-size: 13pt"  face= "Times New Roman"><?php echo $ex_lct_caseyear[$index2]; ?> </b> <?php } ?>
   
       <?php
         
          } 
           $lwct_diary_no[$index2]=$get_diary_case_type;
 ...
 <p style="margin: 0px;padding: 0px 0px 0px 2px;clear: both"><b><font style="font-size: 13pt"  face= "Times New Roman" >Sir,</font></b></p>  
      ...
      <p style="color: #000000;text-indent: 40px;padding: 0px 2px 0px 2px;margin: 10px 0px 20px 0px;clear: both" align="justify">
        <font  style="font-size: 13pt"  face= "Times New Roman">
    I am directed to inform you that Review Petition above mentioned filed by you was circulated before Hon'ble
    Judge in Chamber and the same has been dismissed by order dated  <b style="font-size: 13pt"  face= "Times New Roman"><?php echo $listed_dt ?></b>
</font>
     </p>
     <?php
