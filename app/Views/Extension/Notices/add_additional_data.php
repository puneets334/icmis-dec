...
?> 
 <tr>
             ...
             <td>
                 ...
                 <input type="checkbox" name="chk_id<?php echo $sno; ?>" id="chk_id<?php echo $sno; ?>" style="background-color:  black" <?php if($ck_en_nt=='1') {  ?> checked="checked" <?php } else if($ck_en_nt=='0') {  ?> checked="checked" <?php } ?> />
            
             </td>
             <td style="width: 23%;" id="td_cell_s<?php echo $sno; ?>">

              <textarea id="sp_nm<?php echo $sno; ?>" style="resize:none;width: 80%" onfocus="clear_data(this.id)"><?php
             if($ck_en_nt=='1') { echo $row_office['name']; } 
                     ?></textarea> 
               
                 <input type="hidden" name="hd_sr_no<?php echo $sno; ?>" id="hd_sr_no<?php echo $sno; ?>" value="<?php if($ck_en_nt=='1') {  echo $row_office['sr_no'] ;} ?>"/>
                  <input type="hidden" name="hd_pet_res<?php echo $sno; ?>" id="hd_pet_res<?php echo $sno; ?>" value="<?php if($ck_en_nt=='1') { echo $row_office['pet_res'] ;} ?>"/>
             </td>
             <td style="width: 23%;">

                     <textarea id="sp_add<?php echo $sno; ?>" style="resize:none;width: 80%" onfocus="clear_data(this.id)"><?php if($ck_en_nt=='1') {  echo $row_office['address'] ;}?></textarea> 
             </td>
             <td style="width: 9%;">
                 <div>
<select name="ddlState<?php echo $sno; ?>" id="ddlState<?php echo $sno; ?>" onchange="getCity(this.value,this.id,'0')" style="width: 120px" onfocus="clear_data(this.id)">
                  ...
                  if($ck_en_nt=='1')
                  {
                           foreach ($get_states as $k2)
                                 {
                               $key2=  explode('^',$k2 );
                                if(preg_match('/[0-9]/', $row_office['state']) && $row_office['state']!=NULL && $row_office['state']!='')
                              {
                               ?>
                     ...
                     <?php
                                  ...
                                  if(($row_office['state']==NULL || $row_office['state']==0) && $ck_en_nt=='1')
                                 {
                                     ?>
               ...
               <?php
                   if($ck_en_nt=='1')
                  {
                       $get_districts=get_citys($row_office['state']);
...
<select name="ddl_nt<?php echo $sno; ?>" id="ddl_nt<?php echo $sno; ?>" style="width: 100%;" onfocus="clear_data(this.id)" <?php if($ck_en_nt=='1' && preg_match('/,/',$ck_en_nt_x['nt_type'])) {  ?> multiple="multiple" <?php ;} ?> onchange="get_wh_p_r(this.value,this.id)">
 ...
 <td style="width: 6%;" >
                 <input type="text" size="9" name="txtAmount<?php echo $sno; ?>" 
                        ...
                        onkeypress="return OnlyNumbersTalwana(event,this.id)" <?php if($ck_en_nt=='1') {?> value="<?php  echo $row_office['amount'];  ?>"  <?php } ?>/>
             </td>
          </tr>
            ...
            <td colspan="7" style="border: 0px;border-color: white">
                <table style="width: 100%" class="c_vertical_align tbl_border">
                  ...
                  <?php
                    ...
                    $del_tw_copysend_to='';
                    $ex_c_st='';
                            //$tw_o_r_s="Select del_type from tw_o_r where tw_org_id='$row_office[id]' and display='Y'";
                            //$tw_o_r_s=mysql_query($tw_o_r_s) or die("Error: ".__LINE__.mysql_error());

                            $tw_o_r_s = $noticesModel->get_tw_o_r_s($row_office);
                            if(!empty($tw_o_r_s))
                            {                                 
                                foreach ($tw_o_r_s as $row3)  
                                 {
                                    if($del_modes=='')
                                        $del_modes=$row3['del_type'];
                                    else
                                         $del_modes=$del_modes.$row3['del_type'];
                                }
                            }
                            
                          /*  $tw_send_to="SELECT a.id, del_type, tw_sn_to, sendto_state, sendto_district, copy_type,send_to_type FROM tw_o_r a
                                         JOIN tw_comp_not b ON a.id = b.tw_o_r_id WHERE tw_org_id = '$row_office[id]' AND a.display = 'Y' AND b.display = 'Y'
                                         AND copy_type =0";
                            $tw_send_to=mysql_query($tw_send_to) or die("Error: ".__LINE__.mysql_error()); */

                            $tw_send_to = $noticesModel->get_tw_o_r($row_office);
                                 if(!empty($tw_send_to))
                                 {
                                    foreach ($tw_send_to as $row4) {
                                         if($del_tw_send_to=='')
                                           ...
                                           $del_tw_send_to=$del_tw_send_to.'#'.$row4['del_type'].'~'.$row4['tw_sn_to'].'~'.$row4['sendto_state'].'~'.$row4['sendto_district'].'~'.$row4['send_to_type'];   
                                     }
                                 }
                                 
                                 /* $tw_cp_send_to="SELECT a.id, del_type, tw_sn_to, sendto_state, sendto_district, copy_type,send_to_type
                                                 FROM tw_o_r a JOIN tw_comp_not b ON a.id = b.tw_o_r_id WHERE tw_org_id = '$row_office[id]'
                                                 AND a.display = 'Y' AND b.display = 'Y' AND copy_type =1 ORDER BY id, del_type, copy_type";
                                  $tw_cp_send_to=mysql_query($tw_cp_send_to) or die("Error: ".__LINE__.mysql_error()); */

                                  $tw_cp_send_to = $noticesModel->get_tw_cp_send_to($row_office);
                                    ...
                                    foreach ($tw_cp_send_to as $row5) {
                                         if($main_id!=$row5['id'])
                                         {
                                          if($del_tw_copysend_to=='')
                                           ...
                                           $del_tw_copysend_to=$del_tw_copysend_to.'#'.$row5['del_type'].'~'.$row5['tw_sn_to'].'~'.$row5['sendto_state'].'~'.$row5['sendto_district'].'~'.$row5['send_to_type'];   
                                    $main_id=$row5['id'];
                                         }
                                         else 
                                         {
                                           ...
                                           $ex_c_st=$ex_c_st.'#'.$row5['del_type'].'~'.$row5['tw_sn_to'].'~'.$row5['sendto_state'].'~'.$row5['sendto_district'].'~'.$row5['send_to_type'];   
                                         }
                                      }
                                 }
                      ...
                      ?>
                            ...
                            <?php
                             if($ck_en_nt=='0' && $mode=='o')
                             {
                                 ...
                                 {
                                 //$dist_court="select id_no from state where ref_code_id='$row_crt_adv[l_dist]' and display='Y'";
                                // $dist_court=  mysql_query($dist_court) or die("Error: ".__LINE__.mysql_error() );
                                // $district_code=  mysql_result($dist_court, 0);
                                 $dist_court=   is_data_from_table('master.state'," ef_code_id='$row_crt_adv[l_dist]' and display='Y' ", "id_no", '');
                                 $district_code= $dist_court['id_no'];
                                 }
                                 else 
                                 {
                                  ...
                                  else if($ck_en_nt==0 && $mode=='o')
                                  {
                                      ...
                                      for ($index4 = 1; $index4 < $t_sno; $index4++) {
                                         ...
                                         $indexx= $index4-1;
                                          ?>
                                    <div style="margin-top: 10px">   
                                    <select name="ddl_send_copy_type<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" id="ddl_send_copy_type<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>">
                                             <option value="">Select</option>
                             <option value="2" >Other</option>
                                <option value="1" <?php if($mode=='o' && $ck_en_nt=='0') { ?> selected="selected" <?php } ?>>Advocate</option>
                                        </select>  
                                         <select name="ddlSendCopyTo_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" id="ddlSendCopyTo_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" onfocus="clear_data(this.id)" style="width: 130px;">
                                             <option value="">Select</option> 
                                            <?php 
                                             if($ck_en_nt=='0' )
                    {
                      ...
                      ?>
                       ...
                       <select name="ddl_cpsndto_state_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" id="ddl_cpsndto_state_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" style="width: 100px" onchange="getCity(this.value,this.id,'3','r')">
                                             <option value="">Select</option>
                                 <?php 
                     ...
                     <?php
                                 ...
                                 ?>
                                        ...
                                        <select name="ddl_cpsndto_dst_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" id="ddl_cpsndto_dst_<?php echo $mode.$sno; ?>_<?php echo $indexx; ?>" style="width: 100px">
                                             <option value="">Select</option>
                                  <?php
