                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <em><?php if($caseDetails['diary_no_rec_date']!='') echo date('d-M-Y',strtotime($caseDetails['diary_no_rec_date'])).' on '.date('h:i A',strtotime($caseDetails['diary_no_rec_date'])); else echo '--';?></em>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label> Registration Date:</label>
                                                        <em><?php if($caseDetails['fil_dt']!='') echo date('d-M-Y',strtotime($caseDetails['fil_dt'])).' on '.date('h:i A',strtotime($caseDetails['fil_dt'])); else echo '--';?></em>
                                                    </div>
                                                </div>
                                            </div>
...
async function getCoram(){
    ...
    $.ajax({
        type: 'POST',
        url: "get_coram",
        data: {date:$("#ndt").val(),heading:$('#heading').val(),board:$("#board_type").val(), CSRF_TOKEN:CSRF_TOKEN_VALUE}
    })
    .done(function(msg){
        $("#coram").html(msg);
        updateCSRFToken()
...
$(document).on("change","#coram",function(){
    ...
    $.ajax({
        type: 'POST',
        url: "set_sitting_jud",
        data: {coram:$(this).val(), CSRF_TOKEN:CSRF_TOKEN_VALUE}
    })
    .done(function(msg){
        var msg2 = msg.split('#');
        $("#sitt_jud").val(msg2[0]);
...
async function getSubhead() {
    ...
    $.ajax({
        ...
        data: {heading:$("#heading").val(),side:$("#side_hd").val(),dno:$("#fil_hd").val(),CSRF_TOKEN:CSRF_TOKEN_VALUE}
    })
    .done(function(msg){
        $("#subhead").html(msg);
        updateCSRFToken()
