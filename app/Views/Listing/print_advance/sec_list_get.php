...
<section class="content" >
    <div class="container-fluid" id="prnnt">
        <div class="row">
            <?php
								...
								<table class="table table-striped custom-table" id="example1">
									...
									if (!empty($results)){ ?>
										...
										if ($mnhead_print_once == 1) {
											...
											if ($main_supp_fl == "2") {
												$print_mainhead = "SUPPLEMENTARY LIST";
											}
											?>
											
												<div  style="text-align: center; text-decoration: underline;"><?php if ($jcd_rp !== "117,210" AND $jcd_rp != "117,198") {
														echo $print_mainhead;
													} ?>
											    </div>
											<thead>
												<tr>
													...
													<th style='text-align:center; width: 260px; '>Case No.</th>
													<th style='text-align:center'>Petitioner / Respondent</th>
											...
											<?php  $mnhead_print_once++; }
													...
													if ($subheading != $subheading_rep) {
													   if ($jcd_rp !== "117,210" AND $jcd_rp != "117,198") {
																echo "<tr><td></td><td></td><td style='font-size:12px; font-weight:bold;  text-align:center;'>" . $subheading . "</td><td></td></tr>";
										...
										$output .= "<tr><td style='text-align:center'>$print_srno</td><td style='text-align:center'>" . $is_connected . "$comlete_fil_no_prt" . "<br/>" . $row['section_name'] ."<br/>" . $row['name'] . "</td><td style='text-align:center'>" . $pet_name . "</td><td style='text-align:center;'>" . str_replace(",", ", ", trim($padvname, ","));
										$output .= "</td></tr>";
										$output .= "<tr><td></td><td></td><td  style='font-style: italic; text-align:center'>Versus</td><td></td></tr>";
										$output .= "<tr><td></td><td></td><td style='text-align:center'";

										$output .= ">" . $res_name . "</td><td style='text-align:center;'>" . str_replace(",", ", ", trim($radvname, ","));
										if ($impldname) {
											$output .= "<br/>" . str_replace(",", ", ", trim($impldname, ","));
										}
									$output .= "</td></tr>";
									if ($mainhead == "M" OR $mainhead == "F") {
										$output .= "<tr><td></td><td></td><td  style='font-weight:bold; color:blue; text-align:center'>";
										if ($row['listorder'] == '4' OR $row['listorder'] == '5')
											$output .= "{" . $row['purpose'] . " for $fix_dt } ";

										$output .= get_cl_brd_remark($diary_no) . "</td><td></td></tr>";
									}
...
<div style="width: 100%; padding-bottom:1px; background-color: #ddf1f9; text-align: center; border-top: 1px solid #000; position: fixed; bottom: 0; left: 0; right: 0; z-index: 0; display:block;" id="block_msg_changer">
       <?php 
    if (!file_exists($filePath)) { ?>
