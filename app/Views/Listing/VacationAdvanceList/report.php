<?= view('header'); ?>
<section class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header heading">
                        <div class="row">
                            <div class="col-md-10">
                                <h3 class="card-title">Vacation Report</h3>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <?php
                        echo form_open('Listing/VacationAdvanceList/VacationReports', ['class' => 'form-horizontal', 'id' => 'push-form', 'method' => 'post']);
                        csrf_token();
                        ?>
                        <div id="dv_content1">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group row" id="decline_remain">
                                        <label for="status" id="lbl_decline_remain" class="col-sm-5 col-form-label">Select</label>
                                        <div class="col-sm-7">
                                        <input class="" type="radio" name="listDecline_remain" id="listDecline_remain" value="D"
                                            <?= (isset($type) && $type === 'D') ? 'checked' : 'checked'; ?>>Declined Matters&nbsp;
                                        <input class="" type="radio" name="listDecline_remain" id="listDecline_remain" value="R"
                                            <?= (isset($type) && $type === 'R') ? 'checked' : ''; ?>>Remaining Matters&nbsp;&nbsp;
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4 row" id="fromToDatePicker">
                                    <div class="col-md-6">
                                        <label for="from_date" id="lbl_from_date">From Date </label>
                                        <input type="text" class="form-control dtp" name='from_date' id='from_date' placeholder="From Date" value="<?=(isset($from_date))?$from_date:date('d-m-Y')?>" />
                                    </div>
                                    <div class="col-md-6">
                                        <label for="to_date" id="lbl_to_date">To Date </label>
                                        <input type="text" class="form-control dtp" name='to_date' id='to_date' placeholder="To Date" value="<?=(isset($to_date))?$to_date:date('d-m-Y')?>" />
                                    </div>
                                </div>    
                                <div class="col-md-2 mt-4">
                                    <input type="submit" id="button_id" class="btn btn-primary" name="button_id" value="Get Report" />
                                </div>
                            </div>
                            <div id="res_loader"></div>
                            <div id="dv_res1"></div>
                        </div>
                        <?php echo form_close(); ?>

    
                        <?php if (!empty($case_result)) { ?>
                            <div class="table-responsive">
                                <div id="printable" class="box box-separator my-4">
                                    <caption>
                                        <h3 class="center mt-10">
                                            <strong> Vacation Advance List - <?php if ($type == 'D') {
                                                                echo " Declined "; ?> Matters List between <?php echo date("d-m-Y", strtotime(strtr($from_date, '/', '-'))); ?> and <?php echo date("d-m-Y", strtotime(strtr($to_date, '/', '-')));
                                                    } else {
                                                        echo " Remaining/Listed Matters Report.";
                                                    } ?>
                                            </strong>
                                        </h3>
                                    </caption>

                                    <table id="reportTable1" class="table table-striped table-hover">
                                        <thead>
                                            <tr>
                                                <th style="width: 5%;" rowspan='1'>SNo.</th>
                                                <th style="width: 20%;" rowspan='1'>Case No @ Diary No.</th>
                                                <th style="width: 35%;" rowspan='1'>Cause Title</th>
                                                <th style="width: 30%;" rowspan='1'>Advocate</th>
                                                <?php
                                                if ($type == 'D') {
                                                ?>
                                                    <th style="width: 30%;" rowspan='1'>Decline On</th>
                                                <?php
                                                }
                                                ?>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
                                            $s_no = 1;
                                            foreach ($case_result as $result) {
                                            ?>
                                                <tr>
                                                    <td><?= $s_no; ?></td>
                                                    <td><?php echo $result['case_no']; ?></td>
                                                    <td><?php echo $result['cause_title']; ?></td>
                                                    <td><?php echo $result['advocate']; ?></td>
                                                    <?php
                                                    if ($type == 'D') {
                                                    ?>
                                                        <td> <?php echo date('m-d-Y h:i:s',strtotime($result['updated_on'])); ?></td>
                                                    <?php
                                                    }
                                                    ?>
                                                </tr>
                                            <?php
                                                $s_no++;
                                            }
                                            ?>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            <?= form_close() ?>
                            <?PHP
                                $case_result = null;
                            } else { ?>

                                <div class="alert alert-info alert-dismissable fade in" id="info-alert">
                                    <button type="button" class="close" data-dismiss="alert">x</button>
                                    <strong>Info! </strong>
                                    No Record Found.
                                </div>
                            <?php  } ?>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<script>
    $(document).ready(function() {
        if($("input[type=radio][name='reportType']:checked").val()=='R'){
            $('#fromToDatePicker').hide();
        }
        $(function() {
            $('input[type=radio][name=listDecline_remain]').change(function() {
                if ($(this).val() == 'R') {
                    $('#fromToDatePicker').hide();
                }
                else {
                    $('#fromToDatePicker').show();
                    $("#from_date").prop('required',true);
                    $("#to_date").prop('required',true);
                }
            });
        });

        $(function () {
            $('.datepick').datepicker({
                format: 'dd-mm-yyyy',
                autoclose:true
            });
        });

        $('#reportTable1').DataTable( {
            /* dom: 'Bfrtip',
             buttons: [
                 'excelHtml5',
                 'pdfHtml5'
             ]*/

            "bProcessing"   :   true,
            dom: 'Bfrtip',
            buttons: [
                'excelHtml5',
                {
                    extend: 'pdfHtml5',
                    pageSize: 'A3',
                    customize: function ( doc ) {
                        doc.content.splice( 0, 0, {
                            margin: [ 0, 0, 0, 5 ],
                            alignment: 'center',
                            image: 'data:image/png;base64,'
                        });
                        doc.watermark = {text: 'SUPREME COURT OF INDIA', color: 'blue', opacity: 0.05}
                    }
                }

            ]

        });
    });

    $("#info-alert").fadeTo(2000, 500).slideUp(500, function(){
        $("#info-alert").slideUp(500);
    });


</script>