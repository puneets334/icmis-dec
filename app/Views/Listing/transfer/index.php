            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header heading">
                        <div class="row">
                            <div class="col-sm-10">
                                <h3 class="card-title">TRANSFER MODULE</h3>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="">
                                    <div class="row">
                                        ...
                                        <div class="col-md-1">
                                                ...
                                                <label class="text-leftALLOCATION MODULE">Is NMD</label>
                                ...
                                <table class="px-5">
                                        <tr>
                                            <td class="jud_all_al" >
                                                <fieldset>
                                                    ...
                                                    <?php if (!empty($roster_judges)): ?>
                                                        <table border="0" width="100%" style="text-align: center; background:#f6fbf0;" cellspacing="1">
                                                            <tr>
                                                                <td>
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" id="md_name" name="md_name" value="transfer"/>
                                            </td>                        
                                        </tr>
                                    </table> 
                            </div>
                            <div id="dv_res1" class="px-5"></div>
                            <div id="dv_res2" class="px-5"></div>
                    </form>
...
<script>
$(document).on("click","#btn_send_pool",function(){
    var pool_dt = $("#pool_date").val();   
    var chk_tr = "";
    $('input[type=checkbox]').each(function (){
    if($(this).attr("name")=="chk_tr" && this.checked)
        chk_tr += $(this).val() + ",";
    });
    if(chk_tr == ''){ $('#dv_res2').html('<table widht="100%" align="center"><tr><td class="class_red">Please select Case No.</table>'); return false;  }
        var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
        $.ajax({
            url: "<?php echo base_url('Listing/Transfer/sent_to_pool/'); ?>",
            cache: false,
            async: true,
            data: {pool_dt: pool_dt, chk_tr:chk_tr,CSRF_TOKEN: CSRF_TOKEN_VALUE},
            beforeSend:function(){
...
$(document).on("change","#coram_from",function(){
    var mainhead = get_mainhead();
    var list_dt = $("#ldates").val();   
    var jud_ros = $("#coram_from").val();
    var board_type =  get_bench();
    //var board_type = 'J';
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
        $.ajax({
            //url: '../common/get_cl_print_partno.php',
            url: "<?php echo base_url('Listing/Transfer/get_cl_print_partno/'); ?>",
            cache: false,
            async: true,
...
$(document).on("change","#tans_to_date",function(){
...
function ddd(list_dt,mainhead,sitting_judges,bench){   
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
        $.ajax({
        url: "<?php echo base_url('Listing/Transfer/get_roster_judges_p/'); ?>",
        cache: false,
        async: true,
...
async function ddd_to_tran(list_dt,mainhead,sitting_judges,bench){   
    await updateCSRFTokenSync();
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    $.ajax({
        url: "<?php echo base_url('Listing/Transfer/get_ros_to_tans_p/'); ?>",
        cache: false,
        async: true,
        data: {list_dt: list_dt, mainhead:mainhead, sitting_judges:sitting_judges, bench:bench, CSRF_TOKEN: CSRF_TOKEN_VALUE},
        beforeSend:function(){
   ...
   function chkall_trans(e) {
        var elm = e.name;
        if (document.getElementById(elm).checked) {
            $('input[type=checkbox]').each(function() {
                if ($(this).attr("name") == "chk_tr")
                    this.checked = true;
            });
        } else {
            $('input[type=checkbox]').each(function() {
