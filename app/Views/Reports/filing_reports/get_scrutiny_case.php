<div class="card">
    ...
    <div class="cl_center" style="font-size: 17px;">SUPREME COURT OF INDIA</div>
                  <?php 
                  //$tot_defects="Select count(id) from obj_save where  display='Y' and diary_no='$dairy_no' group by diary_no";
                  //$tot_defects=mysql_query($tot_defects) or die("Error: ".__LINE__.nysql_error());
                  //$res_tot_defects=mysql_result($tot_defects,0);
                  ?>
                    ...
                    <div class="cl_center" style="float: right">Total Defects <?php //echo $res_tot_defects; ?></div>
                    <div class="cl_center" style="margin-top: 5px;clear: both;font-size: 17px;font: bolder">Diary No. <b style="font-size: 17px"><?php //echo $_REQUEST[d_no] ?>-<?php //echo $_REQUEST[d_yr]; ?></b></div>
            ...
            <?php if(!empty($dreports)):?>
                ...
                <thead>
                    ...
                    <td><a target="_blank" href="<?php echo base_url() ?>/Reports/Filing/Filing_Reports/scrutiny_user_wise_detail_report/<?= $row['usercode'];?>/<?=$row['save_date'];?>/<?=str_replace(array(' ','.'),'_',$row['name']);?>"> <?=$row['total'];?></a></td>
                    </tr>
