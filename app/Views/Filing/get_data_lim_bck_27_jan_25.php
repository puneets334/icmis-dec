...
if($res_p_r) {
    ...
    if($ch_cat) {
        ...
        //pr($ch_cat);
        //foreach($ch_cat as $row){
            if(isset($r_chk_limi['order_by']) && ($r_chk_limi['order_by']<$order_by)) {
                $ck_status=1;
                //pr('d');
                //$cat=$row['cat'];
                //$sub_cat=$row['subcat'];
                //$sub_sub_cat=$row['subcat1'];
                $category_code=" casetype_id='$res_p_r[casetype_id]' and submaster_id='$ch_cat[submaster_id]' and case_law=0";
    ...
    if($ck_status==0) {
        ...
        if($res_chk_limi>0) {
           $ck_status=3;
            $category_code=" casetype_id='$res_p_r[casetype_id]' and  submaster_id='0' and case_law=0";
        }
    }
...
//pr($ck_status);
    if($ck_status==2) {
        ...
        if($ch_cat) {
            $order_by=1;
            $ck_status=4;
            if($a_chk_limi['order_by']<$order_by) {
...
//$no_rws =  !empty($no_rws) ? count($rw_sq) : 0;
//pr($no_rws);
//echo ($ck_status);
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                                ...
                                $o_c='';
                                $o_period=0;
                                //$row1 = $limitModel->getLimitationForClaim($category_code); 
                                
                                $claim = $limitModel->getLimitationForClaim($category_code); 
                               
                                foreach($claim as $row1)
                                {
                                        ...
                                        if(isset($row1['order_cof']) && ($row1['order_cof'] == 'O')) {
                                        $o_cof='rdn_o';
                                        $o_c="Order";  
                                        $o_period=$row1['limitation'];
                                    } elseif(isset($row1['order_cof']) && $row1['order_cof']=='C') {
                                            $o_cof='rdn_c';
                                    ...
                                    <input type="radio" name="rdn_o_cof" id="<?php echo $o_cof; ?>" value="<?php echo $row1['order_cof']; ?>"
                                            <?php if($row1['order_cof']=='O' && $no_rws==0) { ?> checked="checked" <?php } else if(isset($rw_sq['order_cof']) && ($rw_sq['order_cof']==$row1['order_cof'])) { ?> checked="checked"  <?php  } ?> class="c_o_cof"/> 
                                    ...
                                    <input type="text" name="txt_order_dt<?php echo $row1['order_cof']; ?>" 
                                        id="txt_order_dt<?php echo $row1['order_cof']; ?>" maxlength="10" size="9" class="dtp"   
                                            <?php if(($row1['order_cof']!='O' && $no_rws==0 && $no_rws==0) || (isset($rw_sq['order_cof']) && $no_rws!=0 && ($rw_sq['order_cof']!=$row1['order_cof'])) ) { ?>  disabled="true" <?php } ?>
                                                value="<?php if($row1['order_cof']=='O' && $no_rws==0){?><?php echo $_REQUEST['sp_lct_dec_dt']; ?><?php ;} else if( isset($rw_sq['order_cof']) && ($rw_sq['order_cof']==$row1['order_cof'])){ echo date('d-m-Y',strtotime($rw_sq['o_d']));}?>"/>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <!--<input type="text" name="climit" id="climit"  readonly="readonly" value="" size="4" disabled="true"/>-->
                        </td>
                    </tr>
                    <tr>
                       <td>
                           ...
                           <?php
                                /*$jailer_sign_dt="Select jailer_sign_dt from  jail_petition_details where diary_no='$dairy_no' and jail_display='Y'";
                                $jailer_sign_dt=  mysql_query($jailer_sign_dt) or die("Error: ".__LINE__.  mysql_error());
                                $jail_dt='';
                                if(mysql_num_rows($jailer_sign_dt)>0){
                                    $r_jailer_sign_dt=  mysql_fetch_array($jailer_sign_dt);
                                    $jail_dt=date('d-m-Y',  strtotime($r_jailer_sign_dt['jailer_sign_dt']));
                                }*/
                           } else {
                           ?>
                            <b>Date of Filing</b>
                        ...
                        <td>
                            <input class="dtp" name="filing_dt" id="filing_dt" type="text" maxlength="10" size="9"  
                                   ...
                                   value="<?php if($res_p_r['nature']==6) { if($no_rws==0) { echo $jail_dt;} else { if($rw_sq['f_d']!='0000-00-00' && $no_rws!=0) echo date('d-m-Y',strtotime($rw_sq['f_d'])) ; } } else { if($no_rws==0) { echo date('d-m-Y',strtotime($res_p_r['diary_no_rec_date'])); } else{ if($rw_sq['f_d']!='0000-00-00' && $no_rws!=0) echo date('d-m-Y',strtotime($rw_sq['f_d'])) ;}} ?>"/>
                        </td>
                    </tr>

                    <!--<tr>
                       <td>
                            <b>Date of Attestation</b>
                        </td>
                        <td>
                            <input class="dtp" name="d_o_a" id="d_o_a" type="text" maxlength="10" size="9"  
                                   onkeyup="chkData_p(event,this.value,this.id)" onkeypress="return OnlyNumbers(event,this.id);" 
                                   value=""/>
                        </td>
                    </tr>-->
                    ...
                    <tr>
                        ...
                        <td>
                            <input class="dtp" name="copy_aply_dt" id="copy_aply_dt" type="text" maxlength="10" size="9"  onkeyup="chkData_p(event,this.value,this.id)" onkeypress="return OnlyNumbers(event,this.id);" value="<?php if(isset($rw_sq['c_d_a']) && $rw_sq['c_d_a']!='0000-00-00' && $no_rws!=0) { echo date('d-m-Y',strtotime($rw_sq['c_d_a'])); } ?>"/>
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            <input class="dtp" name="copy_dlvr_dt" id="copy_dlvr_dt" type="text" maxlength="10" size="9"  onkeyup="chkData_p(event,this.value,this.id)" onkeypress="return OnlyNumbers(event,this.id);" value="<?php  if(isset($rw_sq['d_o_d']) && $rw_sq['d_o_d']!='0000-00-00' && $no_rws!=0)  echo date('d-m-Y',strtotime($rw_sq['d_o_d'])) ?>"/>
                        </td>
                     ...
                     <tr>
                        ...
                        <td>
                            <input class="dtp" name="txt_attestation" id="txt_attestation" type="text" maxlength="10" size="9"  onkeyup="chkData_p(event,this.value,this.id)" onkeypress="return OnlyNumbers(event,this.id);" value="<?php  if(isset($rw_sq['d_o_a']) && $rw_sq['d_o_a']!='0000-00-00' && $no_rws!=0)  echo date('d-m-Y',strtotime($rw_sq['d_o_a'])) ?>"/>
                        </td>

                    </tr>
<!--                    <tr>
                                                        <td>
                            <b>Vacation/Holidays </b>
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            <input name="holiday" type="text" id="holiday"/>
                        </td>
                        <td>
                            <b>Compliance Period (in days)</b>
                        </td>
                        <td>
                            <input name="comp_period" id="comp_period" value="<?php //echo $rw_sq['c_p'] ?>" type="text" size="4"/>
                        </td>
                    ...
                    -->             <tr style="display: <?php if($no_rws>0) { echo 'table-row'; } else { echo 'none';} ?>">
                        <th colspan="2">
                           <div style="text-align: center;margin: 10px">
                                ...
                                <?php echo isset($rw_sq['descr']) ? $rw_sq['descr']: '' ?>
                            </div> </th>
                                     </tr>      <!--
                        </td>
                    </tr>-->
<!--                    <tr>
                    ...
                    <tr>
                        <td colspan="4" style="text-align: center">
                            ...
                            ?>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <input type="hidden" name="hd_lim_id" id="hd_lim_id" value="<?php echo isset($rw_sq['id']) ? $rw_sq['id'] : '' ?>"/>
