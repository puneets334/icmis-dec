...
{ 
    ...
    <div class="box box-danger">
        <caption><h4 style="text-align: center;">  List of Consigned Cases -  Disposed between &nbsp;<strong><?=date('d-m-Y',strtotime($param[0]));?></strong> and <strong><?=date('d-m-Y',strtotime($param[1]));?></strong>, Total Cases :<strong><?=$param[2];?></strong></h4></caption>
        <?php
        if($app_name=='ripeCases')
        
        {
            ...
            <table id="reportTable1" class="table table-striped table-hover table-bordered">
                ...
                <tr>
                    ...
                    <th>Case Details</th>
                    <th>Cause Title</th>
                    <th>Disposal Order Date</th>
                    <th>Consignment <br>Date</th>
                    <th>Bench</th>
                    <th>No of Days <br>since Dispose date</th>
                </tr>
                </thead>
                <tbody>
                <?php
                $s_no=1;
                foreach ($ripeCasesReports as $result)
                {
                    ?>
                    <tr>
                        <td><?= $s_no;?></td>
                        ...
                        <td  width="20%"><?=$result['coram'];?></td>
                        <td  width="10%">
                            <?php
                            $case_remarks=(!empty($result['case_remark']))?$result['case_remark']:null;
                            ?>
                            <?=$result['no_of_days_since_disposal_date'];?><br><?=$case_remarks;?></td>
                    </tr>
                    <?php
                    $s_no++;
                }   //for each
