            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header heading">
                        <h3 class="mb-0">Management Reports >> Uploaded Orders/Judgement List >> Report</h3>
                    </div>
                    <div class="card-body">

                        <?= session()->getFlashdata('msg'); ?>
                        <form class="form-horizontal" id="push-form" method="post" action="<?= base_url(); ?>/ManagementReports/Report/UploadedJudgmentOrdersList">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="usercode" id="usercode" value="<?= session()->get('dcmis_user_idd'); ?>">
                            <div class="form-group row">
                                <label class="col-md-2">Select Order Type:</label>
                                <div class="col-md-4">
                            ...
                            <div class="form-group row">
                                    <label class="col-md-2" for="from_date"> Date From:</label>
                                    <div class="col-md-2">
                                        <input type="text" id="fromDate" name="fromDate" class="form-control datepick" placeholder="From Date" value="<?= set_value('fromDate') ?>">
                                    </div>
                            </div>
                            <div class="form-group row">
                                    <label class="col-md-2" for="to_date">Date To:</label>
                                    <div class="col-md-2">
                                        <input type="text" class="form-control datepick" id="toDate" name="toDate" placeholder="To Date" value="<?= set_value('toDate') ?>">
                                    </div>
                            </div>
                            <div class="form-group row">
                                    <div class="col-md-2"></div>
                                    <div class="col-md-2">
                                        <button type="submit" id="btnGetCases" class="btn btn-info form-control" onclick="this.innerText='Loading...';">Get Cases</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <?php
                        if ($app_name == 'UploadedJudgmentOrdersList') {
                            $string1 = "";
                            if ($param[0] == 1) {
                                $string1 = "Judgments Uploaded ";
                            } elseif ($param[0] == 2) {
                                $string1 = "Orders Uploaded ";
                            }
                        ?>
                <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                            <caption>
                                <h4 style="text-align: center;">
                                    <?= $string1; ?> </strong> BETWEEN &nbsp;<strong><?= date('d-m-Y', strtotime($param[1])); ?></strong> and <strong><?= date('d-m-Y', strtotime($param[2])); ?></strong> </h4>
                                </h4>
                            </caption>
                            <table id="reportTable1" class="table table-striped table-hover box box-danger">
                                ...
                                <tbody>
                                    <?php
                                    $s_no = 1;
                                    foreach ($uploadedOrdersJudgmentsList as $index => $result) {
                                        $noOfPages = '';
                                        $target_pdf = $result['path'];
                                        //echo $target_pdf;
                                        //exec("chmod -R 771 %s",$target_pdf, $output);
                                        $noOfPages =  exec('pdfinfo "' . $target_pdf . '" | grep Pages:| cut -c 17-', $output, $return);
                                        // echo $noOfPages;
                                    ...
                                    ?>
                                        <tr>
                                            <td><?= $s_no; ?></td>
                                            <td>
                                                <a href="#" type="button" class="" data-toggle="modal" data-target="#myModal" id="<?= $result['diary_no']; ?>">
                                            ...
                                            <td> <a href="<?= $result['disp_path']; ?>" target="_blank">View</a> </td>
                                            <td><?= $noOfPages; ?></td>
