...
<div class="card-body">
<?php if(!empty($view_cases_Result)){
	if($view_cases_Result[0]['short_description']!='') 
		   $one_td =  $view_cases_Result[0]['short_description']; 
	 else 
		 $one_td = " UN-REGISTERED CASES";
	
	
$send_title = $one_td.' OF SECTION '.$section.' AS ON: '.date("d-m-Y"); 
?>
<h3 style="text-align: center;"> <strong><?php if($view_cases_Result[0]['short_description']!='') echo $view_cases_Result[0]['short_description']; else echo " UN-REGISTERED CASES";?></strong> OF SECTION <?php echo $section ?> AS ON: <b> <?php echo date("d-m-Y"); ?></b></h3>
    ...
    <thead>
       <tr>
		...
		<tbody>
			<?php
			$s_no = 1;
			foreach ($view_cases_Result as $result) {?>
				<tr>

					<td><?php echo $s_no; ?></td>
					<td><?php echo date('d-m-Y', strtotime($result['diary_no_rec_date']));?></td>
					<td><?php echo date('d-m-Y', strtotime($result['active_fil_dt']));?></td>
					<td><?php echo $result['reg_no_display'].'@DNo.'.$result['diary_number']."/".$result['diary_year']; ?></td>
