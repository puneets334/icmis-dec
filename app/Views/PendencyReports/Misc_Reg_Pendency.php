...
<div class="card-body">
<?php if(!empty($Total_pendency)){ ?>
<h3 style="text-align: center;">Matters of Section <b><?php echo $section ;?> AS ON: <?= date('d-m-Y')?></b></h3>
    ...
    <thead>
       <tr>
		...
		<tbody>
			<?php
			$s_no = 1;
			foreach ($Total_pendency as $result) {?>
				<tr>
     ...
     $(function () {
		...
		var t= $('#example1').DataTable( {
            ...
            buttons: [
                {
                    extend: 'excel',
                    title: "Matters of Section <?php echo $section ;?> As ON <?= date('d-m-Y')?>",
                    exportOptions: {
                        columns: [0,1,2,3,4,5],
                        stripHtml: true
                    }
