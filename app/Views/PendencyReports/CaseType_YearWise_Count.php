 ...
 <table class="table table-striped custom-table" id="example1">
    <thead>
       <tr>
		...
		<tbody>
				<?php $s_no = 1;  foreach ($YearWise as $result) { ?>
					<tr>

						<td><?php echo $s_no; ?></td>
						<td><?php echo $result['diary_year']; ?></td>
						<td><a style="color: #0d48be;" href="<?=base_url();?>/PendencyReports/Physical_verify/View_Cases_Result?casetype=<?php echo $result['active_casetype_id'];?>&sect=<?php echo $result['section_name'];?>&diary_year=<?php echo $result['diary_year'];?>" target="_blank"><?php echo $result['total_pendency'];?></td>
					</tr>
					<?php $s_no++; }  
				?>
	</tbody>
 ...
 <table class="table table-striped custom-table" id="example2">
    <thead>
       <tr>
		...
		<tbody>
					<?php $s_no=1;
					   foreach ($agency_type as $result){ ?>
						<tr>
                            <td><?php echo $s_no;?></td>
							<td><?php if($result['from_court']== '4'){
									echo "(Supreme Court)".$result['agency_name'];
								}else {
									echo $result['agency_name'];
								}?></td>
							<td><a style="color: #0d48be;" href="<?=base_url();?>/PendencyReports/Physical_verify/View_Cases_Result?casetype=<?php echo $result['active_casetype_id'];?>&ref_id=<?php echo $result['ref_agency_code_id'];?>&sect=<?php echo $result['section_name'];?>" target="_blank"><?php echo $result['total_pendency'];?></td>
