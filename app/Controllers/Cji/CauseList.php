...
class CauseList extends BaseController
{
    ...
    public function cause_list_process()
    {
        ...
        $builder->distinct()
            ...
            ->groupEnd()
            ->groupStart()
            // Handle cases where to_date is null or invalid
            ->where('COALESCE(CAST(r.to_date AS date), \'9999-12-31\')', '9999-12-31')
            ->where('CAST(r.from_date AS date)', $formatted_list_date)
            ->orGroupStart()
            // Ensure formatted_list_date is a valid date and compare it with from_date and to_date
            ->where("'$formatted_list_date' BETWEEN CAST(r.from_date AS date) AND COALESCE(CAST(r.to_date AS date), '9999-12-31')", null, false)
            ->groupEnd()
            ->groupEnd()
            ->groupBy('rj.roster_id, r.m_f, r.courtno, mb.board_type_mb')  // Group by all non-aggregated fields
            ->orderBy('r.m_f')
            ->orderBy('min_brd_no');
        // Execute the query
        $query = $builder->get();
        ...
        foreach ($rosterResults as $res) {
            pr($res);
            $title = "Daily Cause List for " . $list_date_dmy . "<br><br>" . $list_day . "<br><br>Court No. ";
            ...
            $sql = "SELECT 
                    ...
                    m.case_status_id
                FROM heardt h
                INNER JOIN main m ON (h.diary_no = m.diary_no AND h.next_dt = '$formatted_list_date' AND h.roster_id = {$res['roster_id']} AND (h.main_supp_flag = 1 OR h.main_supp_flag = 2))
                LEFT JOIN subheading s ON s.stagecode = h.subhead
                LEFT JOIN mul_category mc ON mc.diary_no = h.diary_no AND mc.display = 'Y'
                LEFT JOIN submaster sm ON sm.id = mc.submaster_id AND sm.display = 'Y'
                ...
                ORDER BY h.brd_slno, 
                         CASE WHEN m.conn_key = m.diary_no THEN '0000-00-00' ELSE 99 END ASC, 
            ...
            if (count($caseResults) > 0) {
                ...
                echo "<div class='text-center'><h5>" . ($res['m_f'] == 2 ? 'REGULAR HEARING' : 'MISCELLANEOUS HEARING') . "</h5></div>";
                echo "<div class='text-center'><h5>$title</h5></div>";

                echo '<div class="col-12 align-center pageCenter">';
                echo '<table class="table table-bordered table-responsive" id="tab">
                    <thead>
                ...
                foreach ($caseResults as $row) {
                    ...
                    if ($res['m_f'] == 1 && $subheading != $subheading_rep) {
                        echo "<tr><td colspan='4' style='font-weight:bold; padding-top:15px; padding-bottom:15px; text-decoration:underline; text-align:center;'>$subheading</td></tr>";
                    ...
                    echo "<tr>
                        ...
                        <td class='align-top text-center'>" . $this->get_last_listed_dt($row['diary_no'], $formatted_list_date) . "</td>
                      </tr>";
                }
