...
class Transfer_cases extends BaseController {
    ...
    public function transfer_cases_save() {
        ...
        if ($request->getPost()) {
            ...
            $sqlUpdate = "UPDATE consent_through_email SET is_deleted = 1, deleted_by = :user_id, deleted_on = NOW(), deleted_ip = :user_ip WHERE next_dt = :next_dt AND roster_id = :old_roster_id AND is_deleted IS NULL";
            $db->query($sqlUpdate, [
                'user_id' => session()->get('dcmis_user_idd'),
                'user_ip' => $_SERVER['REMOTE_ADDR'],
                'next_dt' => $next_dt,
