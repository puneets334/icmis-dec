...
class Efiling_webservices {
    ...
    public function webservice($est_code = '') {
        $web_response = curl_get_contents(env('WEB_SERVICE_BASE_URL') . $est_code);
...
/* Prisoner Module */
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/jailDetails/?jailCode=$jailCode");
        if ($data != false) {
    ...
    {
        $details=file_get_contents(env('API_PRISON') . "/prison_contacts/".$jailCode);
        if ($details != false) {
    ...
    {
        $details=file_get_contents(env('API_PRISON') . "/prison_list_response/".$jailCode);
        if ($details != false) {
            return json_decode($details,true);
        } else {
            return NULL;
    ...
    {
        $details=file_get_contents(env('API_PRISON') . "/prisoner_response/".$jailCode."/".$prisonerId."/1/1");
        if ($details != false) {
            return json_decode($details,true);
        } else {
            return NULL;
    ...
    public function OpenAPIwebservice_MultiCINO($est_code = '') {
        $url = env('OPENAPI_URL_MULTIPLE') . $est_code;
        $web_response = curl_get_contents($url . $est_code);
    ...
    public function webservice_get_form_cis($url = '') {
        $url = env('WEB_SERVICE_BASE_URL') . $url;
    ...
    public function getCaseType($national_code, $efiling_for_type_id, $est_state_code) {
        ...
        if (!empty($national_code)) {
            ...
            $encrypt = openssl_encrypt($input_str, 'AES-128-CBC', env('OPENAPI_KEY'), OPENSSL_RAW_DATA, OPENAPI_IV);
            $request_str = base64_encode($encrypt);
            $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
            $est_code = 'getCaseType.php/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function getCaseHistory_cnr_num($cino, $court_type, $est_state_code) {
        if (!empty($cino)) {
            ...
            $data = curl_get_contents(env('WEB_SERVICE_BASE_URL') . "listOfCasesWebService.php?cino=" . $cino . '&court_type=' . $court_type . $est_state_code_param);
    ...
    public function caseHistoryWebService($state_code, $dist_code, $court_code, $case_type, $case_number, $year, $court_type, $est_state_code) {
        ...
        $data = curl_get_contents(env('CASE_NO_WEB_SERVICE_BASE_URL') . "caseNumberSearch.php?state_code=" . $state_code . '&dist_code=' . $dist_code . '&court_code=' . $court_code . '&case_type=' . $case_type . '&case_number=' . $case_number . '&year=' . $year . '&court_type=' . $court_type . $est_state_code_param);
        if ($data != false) {
            return $data;
        } else {
            return NULL;
    ...
    public function getOpenAPIState() {
        $input_str = urlencode('');
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'estlocation/state/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_token=' . $request_token;
        $data = $this->OpenAPIwebservice($est_code);
    ...
    public function getOpenAPIDistrict($state_id) {
        ...
        $encrypt = openssl_encrypt($input_str, 'AES-128-CBC', env('OPENAPI_KEY'), OPENSSL_RAW_DATA, env('OPENAPI_IV'));
        $request_str = base64_encode($encrypt);

        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'estlocation/district/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function getOpenAPIEstablishment($state_id, $dist_code) {
        ...
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'estlocation/courtComplex/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function getOpenAPICourtNameList($establishment_code) {
        ...
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'master/courtList/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function getOpenAPICASE_TYPE($estcode) {
        ...
        $request_str = base64_encode($encrypt);
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'master/caseType/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function getOpenAPICNRSearch($cino) {
        ...
        $request_str = base64_encode($encrypt);
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'casesearch/cnrFullCaseDetails?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
        $data = $this->OpenAPIwebservice_CINO($est_code);
        $result = array($data);
        $cnr_list = array();
        if (!empty($result)) {
    ...
    public function getOpenAPIcaseHistoryWebService($establishment_id, $case_type_id, $case_number, $case_year) {
        ...
        $request_str = base64_encode($encrypt);
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'casesearch/caseNumber?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
        $data = $this->OpenAPIwebservice_CINO($est_code);
        $result = array($data);
    ...
    public function getOpenAPIFilingcaseHistoryWebService($establishment_id, $case_type_id, $filing_number, $case_year) {
        ...
        $request_str = base64_encode($encrypt);
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'casesearch/filingNumber/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
        $data = $this->OpenAPIwebservice_CINO($est_code);
        $result = array($data);
    ...
    public function get_business_details($cino, $business_date) {
        ...
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'casesearch/business/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function show_order_detail_pdf($cino, $order_no, $order_date) {
        ...
        $est_code = 'casesearch/order/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
        $url = env('OPENAPI_URL') . $est_code . '&version=' . env('OPENAPI_VERSION');
    ...
    public function get_cause_list_by_adv_bar($establishment_code, $adv_bar_code, $cause_list_date) {
        ...
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'causelist/advocate/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    public function get_cause_list_by_court_no($establishment_code, $court_no, $cause_list_date, $ci_cri) {
        ...
        $request_token = hash_hmac('sha256', $input_str, env('OPENAPI_HASHHMAC_KEY'));
        $est_code = 'causelist/court/?dept_id=' . env('OPENAPI_DEPT_NO') . '&request_str=' . urlencode($request_str) . '&request_token=' . $request_token;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/updateFilTrapAORtoFDR/?efiling_no=$efiling_no");
        /*if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    public function get_new_case_efiling_scrutiny_cron_SCIS($efiling_nums_str) {
       //echo $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/BulkGetScrutinyStatus/?input=$efiling_nums_str");
       ...
       $data = file_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/BulkGetScrutinyStatus", false, $context);
        if ($data != false) {
    ...
    public function get_new_case_efiling_nums_status_from_SCIS($efiling_nums_str) {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/?input=$efiling_nums_str");
    ...
    public function get_misc_doc_ia_efiling_nums_status_from_SCIS($efiling_nums_str) { 
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/docDetailWithEFilingNo/?input=$efiling_nums_str");
        //$data = file_get_contents("/home/praveen/Desktop/sci-json/search_case_json.txt");
        if ($data != false) {
            return json_decode($data);
        } else {
    ...
    public function get_case_diary_details_from_SCIS($diary_no, $diary_year) {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/caseDetails/?searchBy=D&diaryNo=$diary_no&diaryYear=$diary_year");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getCaseDetails/?diaryNo=$diaryNo");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    public function get_advocate_list($diary_no_with_year){
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getAdvocateDetails/?diaryNo=$diary_no_with_year");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    public function get_case_details_from_SCIS($case_type_id, $case_no, $case_year) {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/caseDetails/?searchBy=C&caseTypeId=$case_type_id&caseNo=$case_no&caseYear=$case_year");
        //$data = file_get_contents("/home/praveen/Desktop/sci-json/diary_reg_search_json_data.txt"
        if ($data != false) {
            return json_decode($data);
        } else {
    ...
    public function get_last_listed_details($diary_no,$diary_year){
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getLastListedDetail/?diary_no=$diary_no&diary_year=$diary_year");
        //$data = file_get_contents("/home/praveen/Desktop/sci-json/diary_reg_search_json_data.txt"
        if ($data != false) {
            return json_decode($data);
        } else {
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getPoliceStation/?state_id=$state_id&district_id=$district_id");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/get_holidays");
        if ($data != false) {
            return json_decode($data);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getLowerCtDetails/?diary_no=$diary_no");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/get_parties/?diary_no=$diary_no");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
...
//        $url1 = env('COURT_ASSIST_URI_ALT')."/t?keys[]=cases.";
...
//    }
    ...
    {
        $url1 = env('API_ICMIS_STATISTICS_URI')."?keys[]=cases.";
    ...
    public function getCitationDetailFromSuplis($type, $param){
        $url=env('URL_FOR_SUPLIS')."request/$type/$param";
        $data = curl_get_contents($url);
        if ($data != false) {
            return json_decode($data);
        } else {
    ...
    {
        $url = env('URL_FOR_LIBRARY_DATA')."/"."request/$type/$param";
        $data = curl_get_contents($url);
    ...
    public function getBarTable($mobile="",$email=""){
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/allAdvocateDetails?mobile=$mobile&email=$email");
        if ($data != false) {
            return json_decode($data);
        } else {
            return NULL;
    ...
    public function getAdvPartyMapping($timestamp=""){
        /*echo env('ICMIS_SERVICE_URL')."/ConsumedData/advPartyMappingDetails?timestamp=".$timestamp;
        exit;*/
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/advPartyMappingDetails?timestamp=".urlencode($timestamp));
        //var_dump($data);
        //exit;
        if ($data != false) {
            return json_decode($data);
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/totalPetitionDetails/?id=$id");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/pendingPetitionDetails/?id=$id");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/disposedPetitionDetails/?id=$id");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getAllPetitionsDetails/?id=$id&status=$val&month=$month&year=$year");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getCertificateCount/?jailcode=$jailcode&status=$status&type=$type");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getCertificateDetails/?jailcode=$jailcode&status=$status&type=$type");
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getvcDetails/?jailcode=$jailcode&fromdate=$fromdate&todate=$todate");
    ...
    {
        /*
         $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/index.php/PutInICMIS/saveIADoc/?table=$table_name&details=$data");
        ...
        return $result = file_get_contents(env('ICMIS_SERVICE_URL').'/PutInICMIS/saveIADoc', false, $context);
    }
    public function generateCaseDiary($encrypted_string){
        ...
        return $result = file_get_contents($url.'/filing/do_generate_case_diary', false, $context);
        /*print_r($result);
        exit();*/
    ...
    {
        $deficitDetails=file_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getDeficitDefectCases/?advocateId=$advocateId");
    ...
    public function getEmpDetailsByempId($empId){
        ...
        if(isset($empId) && !empty($empId)){
            $res =file_get_contents(env('ICMIS_SERVICE_URL').'/ConsumedData/sci_user_details?emp_id='.$empId);
...
//    }
    ...
    public function getAdvPartyMappingBydiaryNo($diary_no=""){
        /*echo env('ICMIS_SERVICE_URL')."/ConsumedData/advPartyMappingDetailsBydiaryNo?diary_no=".$diary_no;
        exit;*/
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/advPartyMappingDetailsBydiaryNo?diary_no=".urlencode($diary_no));
        //var_dump($data);
       // exit;
        if ($data != false) {
            return json_decode($data);
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getLastListedDetail?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year));
        if ($data != false) {
            return json_decode($data);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getCaseDefectsList?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year));
        if ($data != false) {
            return json_decode($data);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getDiaryUserCodeFromICMIS");
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    {
        $data = curl_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/getCaseDetailsWithCaseType?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year));
        if ($data != false) {
            return json_decode($data,true);
        } else {
            return NULL;
    ...
    {
        $data = file_get_contents(env('ICMIS_SERVICE_URL')."/ConsumedData/checkCaseIsEligibleForRefiling?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year));
        if ($data != false) {
            return json_decode($data);
        } else {
            return NULL;
    ...
    {
        //echo env('ICMIS_SERVICE_URL')."/ConsumedData/checkCaseIsEligibleForInsertViaCRON?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year)."&refiled_on=".urlencode($refiled_on)."&refiled_by=".urlencode($refiled_by).'<BR>';
        $url=env('ICMIS_SERVICE_URL')."/ConsumedData/checkCaseIsEligibleForInsertViaCRON?diary_no=".urlencode($diary_no)."&diary_year=".urlencode($diary_year)."&refiled_on=".urlencode($refiled_on)."&refiled_by=".urlencode($refiled_by);
