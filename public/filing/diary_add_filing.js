...
// var host_docroot='https://localhost/supreme_court/public/';
...
function getAdvocate_for_main(id,flag)
{
    if (flag == 'P' )
    {
        ...
        var url = baseURL + "/filing/Diary/get_adv_name" + "?advno=" + document.getElementById('padvno').value + "&advyr=" +
    ...
    else if (flag == 'R')
    {
        ...
        var url = baseURL + "/filing/Diary/get_adv_name" + "?advno=" + document.getElementById('radvno').value + "&advyr=" +
...
function get_a_d_code(id)
{
    ...
    var url = baseURL + "new_filing_autocomp_" + id2[1] + ".php?falagofpost=code&val=" + document.getElementById(id).value;
    xmlhttp.open("GET", url, false);
    xmlhttp.send(null);
}
...
//alert(ext_address);
    var url = baseURL + "/Filing/Diary/save_new_filing?controller=I"  + "&st_status=" + st_status+
        "&ddl_st_agncy="+ddl_st_agncy+"&ddl_bench="+ddl_bench+"&hd_mn="+hd_mn+"&cs_tp="+cs_tp+"&txtFNo="+txtFNo+"&txtYear="+txtYear+
...
function getDistrict(side, id, val,hd_city) {
    ...
    xmlhttp.open("GET", baseURL + "/Common/Ajaxcalls/get_districts?state_id=" + val, true);
    xmlhttp.send(null);
}
...
function get_sc_District(side, id, val,hd_city) {
    ...
    xmlhttp.open("GET", baseURL + "/Common/Ajaxcalls/get_districts?state_id=" + val+"&city="+hd_city, true);
    xmlhttp.send(null);
}
...
//           $('#selct').attr('disabled',true);
    ...
    var url = baseURL + "get_filing_mod_efil.php?txt_p_no=" + txt_p_no + "&txt_yr=" + txt_yr;
    xmlhttp.open("GET", url, false);
    xmlhttp.send(null);
}
...
//    }
    ...
    var url = baseURL + "/Filing/Diary/save_new_filing?controller=I" + '&txt_p_no=' + txt_p_no + '&txt_yr=' + txt_yr + "&st_status=" + st_status;
...
//          alert(idd);
        ...
        $.ajax({
            url: baseURL + '/Filing/Diary/additional_address',
            cache: false,
            async: true,
...
function get_benches(str)
{
...

    {
        ...
        $.ajax({
            url: baseURL +'/Common/Ajaxcalls/get_hc_bench_list',
            //url: 'get_bench.php',
            cache: false,
            async: true,
...
function get_sc_Details()
{
    ...
    $.ajax({
        url: baseURL + '/Filing/Diary/get_sc_parties',
        cache: false,
        async: true,
...

{
    ...
    var url = baseURL + "/Filing/Diary/getsection?q="+data;
...
//alert(" this is save button");
    ...
    if((dno !=''))
    {
        ...
        data1=dno+dyr;
        url = baseURL + "/Filing/Diary/getdiary_no?q="+data1;

    }
    else

    {
        url = baseURL + "/Filing/Diary/getdiary_no?q=" + data;
    }
    //get_sc_Details();
    if (window.XMLHttpRequest)
    {
