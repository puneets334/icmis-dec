$(document).ready(function () {
 ...
 notice_pnt(hd_link);
    ...
    function customConfirmWithReasonModal(message, callback) {
        ...
        modal.addEventListener('close', () => {
            const result = modal.returnValue === 'confirm';
            const reason = result ? modal.querySelector('#reason').value : null;
    ...
    $(document).on('click', '.cl_unlink', function () {
        ...
        customConfirmWithReasonModal('Enter reason for unlinking:', (confirmed, reason) => {
            if (confirmed) {
                console.log('Unfollowed with reason:', reason);
                $.ajax({
                    url: '/Caveat/Similarity/get_diary_unlinked',
                    cache: false,
                    async: true,
                    data: { hd_caveat_no: hd_caveat_no, hd_linked_no: hd_linked_no, sp_cav_diary_lnl_dt: sp_cav_diary_lnl_dt, reason: reason },
                    type: 'POST',
                    success: function (data, status) {
                        updateCSRFToken();
                        alert(data);
