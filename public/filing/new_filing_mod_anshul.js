...
//        }
    ...
    if(pet_type!="I")
    {
        ...
        if(pet_post.value=='')
        {
            alert('Please Enter Petitioner Post');pet_post.focus();return false;
        }
        if(pet_deptt.value=='')
        {
            alert('Please Enter Petitioner Department');pet_deptt.focus();return false;
        }
        if(pet_add.value=='')
        {
            alert('Please Enter Petitioner Address');pet_add.focus();return false;
...
//        }
    ...
    if(res_type!="I")
    {
        ...
        if(res_post.value=='')
        {
            alert('Please Enter Respondent Post');res_post.focus();return false;
        }
        if(res_deptt.value=='')
        {
            alert('Please Enter Respondent Department');res_deptt.focus();return false;
        }
        if(res_add.value=='')
        {
            alert('Please Enter Respondent Address');res_add.focus();return false;
...
//    alert(hd_r_barid+'!'+hd_p_barid);
    ...
    var url = "save_new_filing.php?controller=U&d_no="+d_no+"&d_yr="+d_yr+"&page="+document.getElementById('case_doc').value+
    ...
    +"&t_pet="+tpet.value+"&t_res="+tres.value+"&type_special="+document.getElementById('type_special').value
...
//          alert(idd);
      ...
      $(document).on('click','.cl_deletesP,.cl_deletesR',function(){
          ...
          {
          var idd=$(this).attr('id');
          var cl_css=$(this).attr('class');
          if(cl_css=='cl_deletesP')
          {
          var btn_deleteP=idd.split('btn_deleteP');
          var hd_main_idP=$('#hd_main_idP'+btn_deleteP[1]).val();
      }
      else   if(cl_css=='cl_deletesR')
          {
          var btn_deleteP=idd.split('btn_deleteR');
          var hd_main_idP=$('#hd_main_idR'+btn_deleteP[1]).val();
      }
          $.ajax({
            url: 'delete_additional_address.php',
            cache: false,
            async: true,
            data: {hd_main_idP: hd_main_idP},
            ...
            success: function(data, status) {
                 ...
                 if(cl_css=='cl_deletesP')
                 {
                     $('.tr_P'+btn_deleteP[1]).css('display','none');
                 }
                 else  if(cl_css=='cl_deletesR')
                 {
                     $('.tr_R'+btn_deleteP[1]).css('display','none');
                 }
