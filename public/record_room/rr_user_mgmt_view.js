$(document).ready(function () {
  ...
  $("#department").change(function () {
    ...
    $.ajax({
      type: "GET",
      url: base_url + "/Record_room/Record/rr_user_mgmt_multiple?key=1",
      data: {
        deptname: val,
        cur_user_type:
          $(
  ...
  $("#section").change(function () {
    ...
    $.ajax({
      type: "GET",
      url: base_url + "/Record_room/Record/rr_user_mgmt_multiple?key=2",
      data: {
  ...
  $("#btnShow").click(function () {
    ...
    if (allotmentCategory == 1) {
      ...
      $.ajax({
        ...
        data: {
          /*auth:$("#authority").val(),auth_name:$("#auth_name").val(),auth_sel_name:$("#auth_name option:selected").text(),*/
          dept: $("#department").val(),
          sec: $("#section").val(),
...
$(document).on("click", "[id^='cl_manage_hall']", function () {
  var num = this.id.split("cl_manage_hall");
  $("#dv_sh_hd").css("display", "block");
  $("#dv_fixedFor_P").css("display", "block");
  $.ajax({
    type: "GET",
...
$(document).on("click", "input[name=user_hall_mapping1]", function () {
  var hall_no = $("#hall_no").val();
  var case_group = $("#case_group").val();

  if (hall_no == "") {
    alert("Hall No can not blank");
    $("#hall_no").focus();
    return false;
  }

  if (case_group == "") {
    alert("Case Group Can not blank");
    $("#case_group").focus();
    return false;
  }

  var rkds_id = hall_no + "_" + case_group;

  var maked_cont =
    "<div class='cl_chk_case' id='r_r_" +
    rkds_id +
    "'>" +
    $("#hall_no option:selected").text() +
    "-" +
...
$(document).on("click", "input[name=add-hall-for]", function () {
  var rkds_cst = $("#r_csty").val();
  var rkds_frmyr = $("#r_frmyr").val();
  var rkds_toyr = $("#r_toyr").val();
  var rkds_frmno = $("#r_frmno").val();
...
$(document).on("click", "input[name=al-rkd-case]", function () {
  ...
  $.ajax({
    type: "GET",
    url: base_url + "/Record_room/Record/rr_user_mgmt_multiple?key=17",
...
$(document).on("click", "input[name=mapp_user_hall]", function () {
  ...
  $.ajax({
    type: "GET",
    url: base_url + "/Record_room/Record/rr_user_mgmt_multiple?key=21",
    data: { total: $("#rkds_cases").val(), da_code: rkds_code },
  })
    .done(function (msg) {
      //alert(msg);
...
$(document).on("click", "input[name=al-hall-case]", function () {
  ...
  $.ajax({
    type: "GET",
    url: base_url + "/Record_room/Record/rr_user_mgmt_multiple?key=20",
    data: { total: $("#rkds_cases").val(), hall_no: hallNo },
  })
    .done(function (msg) {
      //alert(msg);
      updateCSRFToken();
...
function removeCase_rkhall(val) {
  var total = $("#rkds_cases").val().split(",");
  var index_val = total.indexOf(val);
  total.splice(index_val, 1);
  $("#rkds_cases").val(total);
...
function allotCase() {
  ...
  $.ajax({
    type: "POST",
    url: $("#hd_folder").val() + "/user_mgmt_multiple.php?key=7",
    data: {
...
function allotCase_rkdcmpda() {
  ...
  $.ajax({
    type: "POST",
    url: $("#hd_folder").val() + "/user_mgmt_multiple.php?key=18",
    data: {
...
function save_judge_info() {
  if (confirm("Are You Sure to Change/Allot This Judge to This User?")) {
    ...
    $.ajax({
      type: "POST",
      url: $("#hd_folder").val() + "/user_mgmt_multiple.php?key=11",
      data: {
        user: $("#the_user").html(),
