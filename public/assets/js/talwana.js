...
$(document).ready(function () {
    ...
    $(document).on('click', '.cl_add_cst', function () {
        ...
        $.ajax({
            url: base_url+"/Faster/FasterController/get_dynamic_cst",
            cache: false,
            async: true,
            data: {
                CSRF_TOKEN: CSRF_TOKEN_VALUE, hd_Sendcopyto_o: hd_Sendcopyto_o, ddlSendCopyTo_o: ddlSendCopyTo_o,
...
function get_send_to_type(idd, id_val, sta, type, d_ta, zx, ddlSendTo_zx) {
    ...
    // alert(city_id);
    // alert('idd:'+idd+' @city_id:ddl_send_copy_type'+type);  // idd:ddl_send_copy_typeo2_0 @city_id:ddl_send_copy_type
    var CSRF_TOKEN = 'CSRF_TOKEN';
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    updateCSRFTokenSync();
    $.ajax({
        url: base_url+"/Faster/FasterController/get_send_to_type",
        cache: false,
        async: true,
        ...
        error: function (xhr) {
            updateCSRFTokenSync();
            alert("ErrorAA: " + xhr.status + " " + xhr.statusText);
        }
...
function getDetails() {
    ...
    document.getElementById('div_results').innerHTML = '<table widht="100%" align="center"><tr><td><img src="'+image_url+'"/></td></tr></table>';
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (document.getElementById('jud1_a')) {
                if (document.getElementById('hd_jud1')) {
            ...
            if (document.getElementById('jud2_a')) {
                if (document.getElementById('hd_jud2')) {
            ...
            if (hd_diary_no && typeof hd_diary_no === 'string') {
                var diary_no = hd_diary_no.slice(0, hd_diary_no.length - 4);
                var diary_yr = hd_diary_no.slice(-4);
            } else {
                var diary_no = '';
                var diary_yr = '';
            }

            $('#t_h_cno').val(diary_no);
            $('#t_h_cyt').val(diary_yr);
        }
    }



    xmlhttp.open("GET", base_url+"/Faster/FasterController/get_notice?d_no=" + d_no + "&d_yr=" + d_yr + "&fno=" + fno + '&ct=' + cstype + '&cn=' + csno + '&cy=' + csyr + '&chk_status=' + chk_status, true);
    xmlhttp.send(null);
}
...
function get_save_dt_bkp() {
    ...
    const d_yr = document.getElementById('t_h_cyt').value;
    
    // Check selected individual or multiple
    $('.cl_ind_mul:checked').each(function() {
        individual_multiple = $(this).val();
    });
    ...
    const hdTot = document.getElementById('hd_tot').value;
    let checkedParty = false;

    for (let y = 0; y < hdTot; y++) {
        const chkId = document.getElementById(`chk_id${y}`);
        if (chkId.checked) {
            checkedParty = true;
            break;
        }
    ...
    const txtFFX = document.getElementById('txtFFX') ? document.getElementById('txtFFX').value : '0';

    if (!checkedParty || !txtFFX) {
        if (!checkedParty) alert("Please check at least one party");
        if (!txtFFX) {
            document.getElementById('txtFFX').style.backgroundColor = 'red';
            alert("Please Select Fixed For");
        }
    ...
    for (let k = 0; k < hdTot; k++) {
        const chkId = document.getElementById(`chk_id${k}`);
        if (chkId.checked) {
            checkOrNt++;

            const sp_nm = document.getElementById(`sp_nm${k}`).value;
            var hd_sr_no = document.getElementById('hd_sr_no' + k).value;
            if (hd_sr_no == '')
                hd_sr_no = '0';
            var hd_pet_res = document.getElementById('hd_pet_res' + k).value;
            var hdinenroll = '';
            var hdinenrollyr = '';
            if (document.getElementById('hdinenroll_' + k))
                hdinenroll = document.getElementById('hdinenroll_' + k).value;
            if (document.getElementById('hdinenrollyr_' + k))
                hdinenrollyr = document.getElementById('hdinenrollyr_' + k).value;
            const sp_add = document.getElementById(`sp_add${k}`).value;
            const ddlNt = document.getElementById(`ddl_nt${k}`);
            let ddl_val = '';

            if (validateFields(k, sp_nm, sp_add, ddlNt, txtFFX)) {
                chkValiRe = 1;
            }

            for (let dd = 0; dd < ddlNt.length; dd++) {
                if (ddlNt[dd].selected) {
                    ddl_val += ddl_val ? `,${ddlNt[dd].value}` : ddlNt[dd].value;
                }
            }
            var txtSub_nm = '';
            const ddlState = document.getElementById(`ddlState${k}`).value;
            const ddlCity = document.getElementById(`ddlCity${k}`).value;
            const del_ty = gatherDeliveryTypes(k);
            var txtAmount = document.getElementById('txtAmount' + k).value;
            var nm_wd = numToString(txtAmount);
            if (document.getElementById('txtSub_nm'))
                txtSub_nm = escape(document.getElementById('txtSub_nm').value);
            var hd_order_date = $('#hd_order_date').val();
            if (typeof hd_order_date === 'undefined'){
                hd_order_date = '';
            }
            const xmlhttp = new XMLHttpRequest();
            var hd_new_upd = 0;
            var hd_mn_id = document.getElementById('hd_mn_id' + k).value;
            xmlhttp.onreadystatechange = function() {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    k++;
                    if (check_or_nt == check_or_nt_ct) {
            ...
            if (ddl_val && del_ty && ddlState && ddlCity) {
                // saveData(k, dNo, dYr, spNm, spAdd, ddlVal, delTy);
                saveData(k, d_no, d_yr, sp_nm, sp_add, ddl_val, del_ty, hd_sr_no, hd_pet_res, ddlState, ddlCity, txtAmount, hd_new_upd, hd_mn_id, nm_wd, txtFFX, txtSub_nm, hdinenroll, hdinenrollyr, hd_order_date, individual_multiple)
            }
        }
    }
...
function validateFields(k, spNm, spAdd, ddlNt, txtFFX) {
    let isInvalid = false;
...
function gatherDeliveryTypes(k) {
    let delTy = '';
    $('.cl_del_mod' + k + ':checked').each(function() {
        const vVal = $(this).val();
        const spId = $(this).attr('id').split('chkOrd')[1];
        const ddlSendTo = $(`#ddlSendTo_${vVal.toLowerCase()}${spId}`).val();
        const ddlSndToState = $(`#ddl_sndto_state_${vVal.toLowerCase()}${spId}`).val();
        const ddlSndToDst = $(`#ddl_sndto_dst_${vVal.toLowerCase()}${spId}`).val();

        if (delTy) delTy += ',';
        delTy += `${vVal}!${ddlSendTo}~${ddlSndToState}~${ddlSndToDst}`;
    });
    return delTy;
}

function saveData(k, d_no, d_yr, sp_nm, sp_add, ddl_val, del_ty, hd_sr_no, hd_pet_res, ddlState, ddlCity, txtAmount, hd_new_upd, hd_mn_id, nm_wd, txtFFX, txtSub_nm, hdinenroll, hdinenrollyr, hd_order_date, individual_multiple) {
    // The saving logic, likely involving AJAX calls
    const xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4) { // Check if the request is complete
            if (xmlhttp.status === 200) { // Check if the status is OK
                const response = xmlhttp.responseText; // Get the response text
                // console.log(response); // Log the response for debugging

                // Process the response as needed
                try {
                    const jsonResponse = JSON.parse(response); // Parse JSON response
                    // Handle success based on the parsed response
                    if (jsonResponse.success) {
                        alert("Data saved successfully!");
                        // Call additional functions if needed, e.g., show_details(d_no, d_yr);
                    } else {
                        alert("Error: " + jsonResponse.message); // Handle any error messages returned from the server
                    }
                } catch (error) {
                    console.error("Error parsing response:", error);
                    alert("Received an invalid response.");
                }
            } else {
                // Handle HTTP errors
                console.error("Error with the request:", xmlhttp.status, xmlhttp.statusText);
...
function getCity(str, idd, sta, type, upd, val_ddlSendTodis) {
    ...
    xmlhttp.open("GET", base_url+"/Faster/FasterController/getCityName?str=" + str, true);
    xmlhttp.send(null);
}
...
function appendRow(tableID) {
    ...
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    $.ajax({
        url: base_url+'/Faster/FasterController/add_additional_data',
        cache: false,
        async: true,
        data: {
            CSRF_TOKEN: CSRF_TOKEN_VALUE, rowcount: rowcount, hd_tot: hd_tot, hd_hd_sec_id: hd_hd_sec_id,
            hd_hd_res_ca_nt: hd_hd_res_ca_nt, hd_n_status: hd_n_status, hd_casetype_id: hd_casetype_id, d_no: d_no, d_yr: d_yr, cstype: cstype, csno: csno, csyr: csyr, t_h_cno: t_h_cno, t_h_cyt: t_h_cyt, chk_status: chk_status
        },
        type: 'POST',
        success: function (data, status) {
            updateCSRFToken();
...
function appendRow_hc(tableID) {
    ...
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    $.ajax({
        url: base_url+'/Faster/FasterController/add_additional_data_hc',
        cache: false,
        async: true,
        data: {
            CSRF_TOKEN: CSRF_TOKEN_VALUE, rowcount: rowcount, hd_tot: hd_tot, hd_hd_sec_id: hd_hd_sec_id,
...
function ck_mul_rem(d_no, d_yr) {
    ...
    xmlhttpp.open("GET", base_url+"/Faster/FasterController/get_ck_mul_rem?d_no=" + d_no + '&d_yr=' + d_yr, true);
    xmlhttpp.send(null);
}
