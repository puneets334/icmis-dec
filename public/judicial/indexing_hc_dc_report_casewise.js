function call_getDetails_new()
{
    ...
    if(validate()==true) {
        var d_no = $('#diary_number').val();
        var d_year = $('#diary_year').val();
        var ct=$('#case_type').val();
        var cn=$('#case_number').val();
        var cy=$('#case_year').val();
        ...
        $.ajax({
            url: base_url+'/Judicial/Hcor/get_indexing_hc_dc_report_casewise',
            type: "GET",
            cache: false,
            async: true,
            beforeSend: function () {
...
function validate()
{
    ...
    if($("input[name='search_type']:checked").val()=='c') {
